                           ...
                           error:(EMError *)error
{
    ...
    else if (callSession.status == eCallSessionStatusAccepted)
    {
        ...
        _timeTimer = [NSTimer weakTimerWithTimeInterval:1.0 target:self selector:@selector(timeTimerAction:) userInfo:nil repeats:YES];
        ...
        if ([self isShowCallInfo]) {
            ...
            if ([_propertyTimer isValid]) {
                [_propertyTimer invalidate];
            }
            
            _propertyTimer = [NSTimer weakTimerWithTimeInterval:2.0 target:self selector:@selector(_reloadPropertyData) userInfo:nil repeats:YES];
        }
    }
}
