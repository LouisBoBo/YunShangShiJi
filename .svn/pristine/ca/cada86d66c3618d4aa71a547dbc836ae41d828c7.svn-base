//
//  CFPageTitleView.m
//  NNJaneBookView
//
//  Created by 柳钟宁 on 2017/8/5.
//  Copyright © 2017年 zhongNing. All rights reserved.
//

#import "CFPageTitleView.h"
#import "GlobalTool.h"

@interface CFPageTitleView ()

@property (nonatomic, strong) UIView *sliderView;
@property (nonatomic, weak) UIButton *selectedButton;

@end

@implementation CFPageTitleView

- (void)titleButtonClicked:(UIButton *)button {
    _selectedIndex               = button.tag;
    if (self.selectedButton) {
    self.selectedButton.selected = YES;
    }
    button.selected              = NO;
    self.selectedButton          = button;

    if (self.buttonSelected) {
        self.buttonSelected(button.tag);
    }

    NSString* title              = self.titles[button.tag];
    CGFloat sliderWidth          = button.titleLabel.font.pointSize * (title.length+5);
//    [self.sliderView mas_remakeConstraints:^(MASConstraintMaker *make) {
//        make.width.mas_equalTo(sliderWidth);
//        make.height.mas_equalTo(2);
//        make.centerX.equalTo(button);
//        make.bottom.equalTo(self).offset(-2);
//    }];
    self.sliderView.frame = CGRectMake(button.frame.origin.x+(button.frame.size.width-sliderWidth)/2, self.frame.size.height-2, sliderWidth, 2);

    [UIView animateWithDuration:0.25 animations:^{
        [self layoutIfNeeded];
    }];
}

- (void)setSelectedIndex:(NSInteger)selectedIndex {
    _selectedIndex   = selectedIndex;
    UIButton* button = self.subviews[selectedIndex];
    [self titleButtonClicked:button];
}

- (void)setTitles:(NSArray *)titles {
    [self.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
    _titles               = titles;
    CGFloat width         = [UIScreen mainScreen].bounds.size.width / titles.count;

    for ( int i           = 0; i<titles.count; i++ ) {
    UIButton* titleButton = [self titleButton:titles[i]];
    titleButton.tag       = i;
        [self addSubview:titleButton];
//        [titleButton mas_makeConstraints:^(MASConstraintMaker *make) {
//            make.top.bottom.equalTo(self);
//            make.left.equalTo(self).offset(width * i);
//            make.width.mas_equalTo(width);
//        }];
        titleButton.frame = CGRectMake(width * i, 0, width, self.frame.size.height);
        if (i != 0) {
    titleButton.selected  = YES;
        } else {
    self.selectedButton   = titleButton;
        }
    }
    UIButton *button      = self.subviews[0];
    NSString *title       = titles[0];
    CGFloat sliderWidth   = button.titleLabel.font.pointSize * (title.length+5);
    [self addSubview:self.sliderView];
//    [self.sliderView mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.width.mas_equalTo(sliderWidth);
//        make.height.mas_equalTo(4);
//        make.centerX.equalTo(button);
//        make.bottom.equalTo(self).offset(-3);
//    }];
    self.sliderView.frame = CGRectMake(button.frame.origin.x+(button.frame.size.width-sliderWidth)/2, self.frame.size.height-2, sliderWidth, 2);

    [self layoutIfNeeded];
}

- (UIButton *)titleButton:(NSString *)title {
    UIButton* titleButton       = [[UIButton alloc] init];
    [titleButton addTarget:self action:@selector(titleButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
    titleButton.titleLabel.font = [UIFont systemFontOfSize:15];
    [titleButton setTitleColor:tarbarrossred forState:UIControlStateNormal];
    [titleButton setTitleColor:tarbarrossred forState:UIControlStateNormal|UIControlStateHighlighted];
    [titleButton setTitleColor:kMainTitleColor forState:UIControlStateSelected];
    [titleButton setTitleColor:kMainTitleColor forState:UIControlStateSelected|UIControlStateHighlighted];
    [titleButton setTitle:title forState:UIControlStateNormal];
    return titleButton;
}

- (void)drawRect:(CGRect)rect {

    [self makelineStartPoint:self.sliderView.frame.origin.y + self.sliderView.frame.size.height andEndPoint:self.sliderView.frame.origin.y + self.sliderView.frame.size.height];
//    [self makelineStartPoint:self.selectedButton.frame.origin.y andEndPoint:self.selectedButton.frame.origin.y];
}

- (void)makelineStartPoint:(CGFloat)statPoint andEndPoint:(CGFloat)endPoint {
    CGContextRef context = UIGraphicsGetCurrentContext();
    CGContextSetLineCap(context, kCGLineCapRound);
    CGContextSetLineWidth(context, 0.5);  //线宽
    CGContextSetAllowsAntialiasing(context, true);
    CGContextSetStrokeColorWithColor(context, kNavLineColor.CGColor);
    CGContextBeginPath(context);

    CGContextMoveToPoint(context, 0, statPoint);  //起点坐标
    CGContextAddLineToPoint(context, self.frame.size.width, endPoint);   //终点坐标
    CGContextStrokePath(context);
}

- (UIView *)sliderView {
    if (!_sliderView) {
        UIView* sliderView            = [[UIView alloc] init];
        sliderView.backgroundColor    = tarbarrossred;
        sliderView.layer.cornerRadius = 2;
        sliderView.clipsToBounds      = YES;
        _sliderView                   = sliderView;
    }
    return _sliderView;
}

@end
