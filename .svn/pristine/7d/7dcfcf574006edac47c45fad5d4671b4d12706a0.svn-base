...
/** 默认参数 */
static const CGFloat JRDefaultColumnCount = 1; // 列数
static const CGFloat JRDefaultRowMargin = 5; // 行间距
static const CGFloat JRDefaultColumnMargin = 5; // 列间距
static const UIEdgeInsets JRDefaultEdgeInsets = {5, 5, 5, 5}; // edgeInsets
...
//    CGFloat height = [self.delegate waterFallLayout:self heightForItemAtNSIndexPath:indexPath width:width];
...
//        if(indexPath.item >0)
//        {
//            totalHeight += [self.columnHeights[indexPath.item-1] floatValue];
//        }
//        y = totalHeight;
//    }
...
- (void)setupAttrsArray
{
    ...
    NSUInteger count1 = [self.collectionView numberOfItemsInSection:1];
    NSUInteger count2 = [self.collectionView numberOfItemsInSection:2];
    ...
    for (int i = 0; i < count2; i++) {
        @autoreleasepool { // 如果item数目过大容易造成内存峰值提高
            UICollectionViewLayoutAttributes *attrs = [self layoutAttributesForItemAtIndexPath:[NSIndexPath indexPathForItem:i inSection:2]];
