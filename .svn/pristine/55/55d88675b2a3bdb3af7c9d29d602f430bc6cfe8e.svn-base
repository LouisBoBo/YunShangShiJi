//
//  FightIndianaDetailViewController.m
//  YunShangShiJi
//
...
@interface FightIndianaDetailViewController ()<DShareManagerDelegate,UISearchBarDelegate, TFScreeningBackgroundDelegate>
...
@implementation FightIndianaDetailViewController
{
    //计时器 天 时 分 秒
    NSString *_day;
    NSString *_hour;
    ...
    UIImageView * yifenImageView;//新手第一次一分钱夺宝
    UIButton *awardButton;  //去领奖
    UILabel *owelPassengers;//剩余团次
    UILabel *countdownlab;  //进度条
    UILabel *countdownback;
    UIView *_sharebackview;
    ManPicDataModel *manPicModel;
    TreasureGroupsModel *group_model;
    
    NSString * indianaShare_link ;
    IndianaPopView * extrabonusview;
}
...
- (void)dataInit
{
    //    NSArray *dataArray = @[@"参与规则：2元即可参与1次夺宝。参与人次达到商品价格后立刻开奖（如本期奖品为价值4000元的iphone7。参与人次达到2000后即开奖）。公布中奖幸运号码并进入下一期夺宝。",@"中奖规则：每次参与成功后即得一个幸运号码。幸运号码为当期夺宝的日期（如17年2月28日即为170228）加当期参与总人数中随机一个号码（如当期奖品为价值4000元的IPHONE7，即可知参与总人次为2000人/次，号码即为0001-2000中随机一个号码）。开奖时在所有幸运号码中随机抽取一个号码中大奖。",@"如何中奖：每分享5次夺宝给自己的微信好友，即可以1分钱参与一次原价2元的夺宝。因每次夺宝总人数是固定的，所以一期夺宝内你参与的夺宝次数越多，你的中奖率就越高。（如当期奖品为价值4000元的IPHONE7，即可知参与总人次为2000人/次，如你参与了200次，则你的中奖率就高达10%）",@"夺宝奖品由衣蝠供应商提供并负责售后，非质量问题，不予以退换货。开奖后的72小时内通过顺丰快递发出。请当面拆箱验货再签收。如发现运输途中造成了商品损坏，请拒签退回。如遇质量问题，请在签收的24小时内联系客服，超时衣蝠将不再承担责任。",@"对于任何通过不正当手段参与夺宝者，衣蝠有权取消其夺宝资格，本次活动最终解释权归衣蝠所有。"];
    //    [self.ruleDataArray addObjectsFromArray:dataArray];
...
{
    FightIndianaPopView *textrabonusview = [[FightIndianaPopView alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight)];
    __weak FightIndianaPopView *view = textrabonusview;
...
{
    NSString *imgUrl = _newimage;
    
    //    NSString *title = [NSString stringWithFormat:@"衣蝠特惠，%@人成团就有机会，%@元团购%@",_ShopModel.group_number,_ShopModel.shop_price,_ShopModel.shop_name];
    ...
    NSString *link  = [NSString stringWithFormat:@"%@/view/activity/signDetail.html?i_c=%@&r=%@&s_c=%@",[NSObject baseH5ShareURLStr],_ShopModel.shop_batch_num,realm,_ShopModel.shop_code];
        ...
        if (responseObject!=nil) {
            ...
            if (responseObject[@"ptdbfx"] != nil ){
                if(responseObject[@"ptdbfx"][@"text"] != nil)
                {
                    ...
                    title = [title stringByReplacingOccurrencesOfString:@"${replace}人" withString:[NSString stringWithFormat:@"%@人",_ShopModel.group_number]];
                    title = [title stringByReplacingOccurrencesOfString:@"${replace}元" withString:[NSString stringWithFormat:@"%.1f元",[_ShopModel.shop_se_price floatValue]]];
                    title = [title stringByReplacingOccurrencesOfString:@"${replace}" withString:[NSString stringWithFormat:@"%@",_ShopModel.shop_name]];
                    
                    NSString *text = responseObject[@"ptdbfx"][@"text"];
                    text = [text stringByReplacingOccurrencesOfString:@"${replace}人" withString:[NSString stringWithFormat:@"%@人",_ShopModel.group_number]];
...
-(void)requestHttp
{
    ...
    if(token == nil)
    {
        ...
        {
            if(self.recordsModel.issue_code != nil || self.issue_code != nil)
            {
                NSString * issue_code = self.recordsModel.issue_code != nil?self.recordsModel.issue_code:self.issue_code;
                url=[NSString stringWithFormat:@"%@shop/queryIndiana?version=%@&shop_code=%@&token=%@&issue_code=%@",[NSObject baseURLStr],VERSION,self.shop_code,token,issue_code];
            }else{
            ...
            {
                NSString * issue_code = self.recordsModel.issue_code != nil?self.recordsModel.issue_code:self.issue_code;
                url=[NSString stringWithFormat:@"%@shop/queryIndiana?version=%@&treas_type=%@&token=%@&issue_code=%@",[NSObject baseURLStr],VERSION,self.shop_code,token,issue_code];
            }else{
        ...
        if (responseObject!=nil) {
            ...
            if(statu.intValue==1)//请求成功
            {
                ...
                if(![dic isEqual:[NSNull null]])
                {
                    ...
                    model.involved_people_num = responseObject[@"shop"][@"involved_people_num"];
                    ...
                    model.banner = responseObject[@"shop"][@"banner"];
                    model.group_number = responseObject[@"shop"][@"group_number"];
                    model.owestatue = [NSString stringWithFormat:@"%@",responseObject[@"ostatus"]];
                    model.in_sum = [NSString stringWithFormat:@"%@",responseObject[@"in_sum"]];
                    _ShopModel=model;
...
{
    if(self.indiaType)//是从H5引导去参团(正在开奖中，已经揭晓不能参团)
    {
        if(_oweStatues.intValue == 0 || _oweStatues ==nil)
        {
            [self acticveIndianaHttp:2];
            [GuideIndianaModel changIndianaDataSuccess:^(id data) {
                //重置引导
            }];
        }
    }else{//如果不是从H5引导便是去开团(正在开奖中，已经揭晓不能开团)
        if(_oweStatues.intValue == 0 || _oweStatues ==nil)
        {
            [self acticveIndianaHttp:1];
        }
    }
}
#pragma mark 参与拼团夺宝
-(void)acticveIndianaHttp:(NSInteger)type
{
    if(type == 2)//夺宝参团
    {
        kWeakSelf(self);
        [FightIndianiaViewModel acticveIndianaHttpShopCode:_ShopModel.shop_code Type:2 ID:self.tuserId Success:^(id data) {
            
            FightIndianiaViewModel *model = data;
            if(model.status == 1)
            {
                [MBProgressHUD show:@"参团成功" icon:nil view:self.view];
                [weakself acticveIndianaHttp:1];//参团成功再去开团
            }else{
        ...
        NSString *tuserId = [[NSUserDefaults standardUserDefaults]objectForKey:USER_ID];
        kWeakSelf(self);
        [FightIndianiaViewModel acticveIndianaHttpShopCode:_ShopModel.shop_code Type:1 ID:tuserId Success:^(id data) {
            
            FightIndianiaViewModel *model = data;
            if(model.status == 1)
            {
                if(self.tuserId.intValue<=0)//正常开团
                {
                    [MBProgressHUD show:@"开团成功" icon:nil view:self.view];
                }else{//如果是H5引导过来开团不提示
                    
                }
                [weakself httpData:self.tuserId.intValue>0?YES:NO];//开团成功刷新拼团详情
            }else{
...
{
    kWeakSelf(self);
    [FightIndianiaViewModel activeRecordHttpShopCode:_ShopModel.shop_code Issue_code:_ShopModel.shop_batch_num Page:_pagecount Success:^(id data) {
        [weakself.MyBigtableview footerEndRefreshing];
        FightIndianiaViewModel *model = data;
        if(model.status == 1)
        {
            [_participateArray addObjectsFromArray:model.data];
            [weakself.MyBigtableview reloadData];
        }
    }];
}
- (void)refreshBigImage
{
    _ShopModel.involved_people_num = [NSString stringWithFormat:@"%zd",_ShopModel.involved_people_num.intValue +1];
    
    _Countdowntime.text = [NSString stringWithFormat:@"已有%d团正在参与",_ShopModel.involved_people_num.intValue];
    
    int activenum =(int)([_ShopModel.active_people_num intValue] - [_ShopModel.num intValue] - _ShopModel.involved_people_num.intValue);
    if(activenum > 0)
    {
        [Signmanager SignManarer].IndianaSurplusCount = activenum;
        owelPassengers.text = [NSString stringWithFormat:@"剩余团次:%d",activenum];
    }else
    {
        [Signmanager SignManarer].IndianaSurplusCount = 0;
        owelPassengers.text = [NSString stringWithFormat:@"剩余团次:%d",0];
    }
    ...
    if(activenum != 0)
    {
        scale = ([_ShopModel.involved_people_num floatValue])/[_ShopModel.active_people_num floatValue];
        if(scale >1)
        {
            scale = 1;
        }
...
#pragma mark 拼团夺宝详情
- (void)httpData:(BOOL)move{
    
    NSString *url = [NSString stringWithFormat:@"rollTrea/getMyRoll?shop_code=%@&issue_code=%@&is_ago=%@&",_ShopModel.shop_code,_ShopModel.shop_batch_num,_oweStatues];
    kSelfWeak;
    [[APIClient sharedManager] netWorkGeneralRequestWithApi:url caches:NO cachesTimeInterval:0 token:YES success:^(id data, Response *response) {
        ...
        if (response.status == 1 && arr.count) {
            [self.fightIndianaData removeAllObjects];
            TreasureGroupsModel *tempModel;//用来记录 （未满团）  自己开的团  参加的团
            for (NSDictionary *dic in arr) {
                ...
                //期号 (==0 时表示没满足人数)
                NSString *issue_code = [NSString stringWithFormat:@"%@",model.issue_code];
                if ([issue_code containsString:@"null"]||[issue_code isEqualToString:@"0"]) {
                    ...
                    [weakSelf.fightIndianaData addObject:model];
            }
            
            //最后（未满团）  自己开的团  参加的团 加入数组
            if (tempModel) {
                [weakSelf.fightIndianaData insertObject:tempModel atIndex:0];
            }
            if (group_model)
            {
                [weakSelf.fightIndianaData insertObject:group_model atIndex:0];
            }
            
            if(weakSelf.fightIndianaData.count)
            {
                _footView=[[UIView alloc]initWithFrame:CGRectMake(0, 0, kApplicationWidth, ZOOM6(100))];
            }else{
                _footView=[[UIView alloc]initWithFrame:CGRectMake(0, 0, kApplicationWidth, kApplicationHeight-Height_NavBar-kUnderStatusBarStartY-55-ZOOM6(940))];
            }
            ...
            if(move)//如果是H5引导过来顶置
            {
                [UIView animateWithDuration:0.5 animations:^{
                    ...
                    if(self.fightIndianaData.count >= 2)//刷新参团参数
                    {
                        TreasureGroupsModel *model = self.fightIndianaData[1];
                        if(_ShopModel.group_number.integerValue-model.num.integerValue == 0)
                        {
                            [self refreshBigImage];
                        }
                    }
                }];
            }
...
-(void)creatHeadview
{
    ...
    [self.AnnouncedBtn setTitle:@"往期揭晓" forState:UIControlStateNormal];
    ...
    self.RecordBtn.frame=CGRectMake(self.AnnouncedBtn.frame.origin.x-ZOOM6(20)-buttonwith, Height_NavBar-40, buttonwith, buttonheigh);
    self.RecordBtn.centerY = View_CenterY(headview);
    self.RecordBtn.backgroundColor = RGBCOLOR_I(249, 36, 61);
    [self.RecordBtn setTitle:@"抽奖记录" forState:UIControlStateNormal];
    [self.RecordBtn setTintColor:[UIColor whiteColor]];
    self.RecordBtn.layer.cornerRadius = 3;
    self.RecordBtn.titleLabel.font = [UIFont systemFontOfSize:ZOOM6(24)];
...
- (void)creatUI
{
    ...
    baoyouBtn.frame = CGRectMake(self.Headimage.frame.size.width-ZOOM6(150)-ZOOM6(20), self.Headimage.frame.size.height-ZOOM6(230), ZOOM6(150), ZOOM6(150));
    
    [baoyouBtn setBackgroundImage:[UIImage imageNamed:@"fightIndiana_guize"] forState:UIControlStateNormal];
...
-(void)creatBIGView
{
    ...
    //    //一分钱夺宝
    //    [yifenImageView removeFromSuperview];
    //    CGFloat imageH = ZOOM6(96);
    //    CGFloat imageW = ZOOM6(390);
    //    yifenImageView = [[UIImageView alloc]initWithFrame:CGRectMake(kScreenWidth-imageW-ZOOM(42), CGRectGetMinY(self.se_price.frame)-ZOOM6(10), imageW, imageH)];
    //    yifenImageView.image = [UIImage imageNamed:@"支付一分钱"];
    //    [self.backview addSubview:yifenImageView];
    
    //查看余额
    [awardButton removeFromSuperview];
    CGFloat awardimageH = ZOOM6(60);
    CGFloat awardimageW = ZOOM6(160);
    awardButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    awardButton.frame = CGRectMake(kScreenWidth-awardimageW-ZOOM(42), CGRectGetMinY(self.se_price.frame)-ZOOM6(10), awardimageW, awardimageH);
    awardButton.backgroundColor = tarbarrossred;
    [awardButton setTitle:@"去领奖" forState:UIControlStateNormal];
    [awardButton setTintColor:[UIColor whiteColor]];
    awardButton.layer.cornerRadius = 3;
    awardButton.hidden = YES;
    awardButton.titleLabel.font = [UIFont systemFontOfSize:ZOOM6(30)];
    [awardButton addTarget:self action:@selector(gotoaward) forControlEvents:UIControlEventTouchUpInside];
    [self.backview addSubview:awardButton];
    ...
    if((_oweStatues.intValue == 0 || _oweStatues.intValue == 1)&& _oweStatues !=nil)
    {
        //倒计时
        ...
        countdownback = [[UILabel alloc]initWithFrame:CGRectMake(ZOOM(42), CGRectGetMaxY(_Countdowntime.frame)+ZOOM6(10), kScreenWidth-2*ZOOM(42), ZOOM6(14))];
        ...
        countdownlab = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, CGRectGetWidth(countdownback.frame)*scale, ZOOM6(14))];
        ...
        totalPassengers.text = [NSString stringWithFormat:@"总需团次:%@",_ShopModel.active_people_num];
        ...
        owelPassengers = [[UILabel alloc] initWithFrame:CGRectMake(kScreenWidth/2, CGRectGetMaxY(countdownback.frame)+ZOOM6(10), kScreenWidth/2-ZOOM(42), ZOOM6(30))];
        owelPassengers.backgroundColor = [UIColor whiteColor];
        owelPassengers.textAlignment = NSTextAlignmentRight;
        
        
        owelPassengers.text = [NSString stringWithFormat:@"剩余团次:%d",[_ShopModel.active_people_num intValue]];
        ...
        if(_myself_incode !=nil && ![_myself_incode isEqualToString:@"(null)"])
        {
            ...
            if(incodes.count > 2)
            {
                tisilab.text = [NSString stringWithFormat:@"你参与了：%d团次\n参与号码:%@,%@...",count.intValue,incodes[0],incodes[1]];
            }else{
        ...
        UILabel *namelab = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(headimage.frame)+ZOOM6(20), ZOOM6(10), ZOOM6(300), ZOOM6(40))];
        namelab.textColor = RGBCOLOR_I(168, 168, 168);
        if(self.recordsModel.in_name == nil)
        {
            namelab.text = @"获奖者:";
        }else
            namelab.text = [NSString stringWithFormat:@"获奖者:%@的团",self.recordsModel.in_name];
        ...
        UILabel *activelab = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(headimage.frame)+ZOOM6(20), CGRectGetMaxY(namelab.frame)-ZOOM6(10), ZOOM6(400), ZOOM6(40))];
        activelab.textColor = kTextColor;
        NSString *myactivenum = [NSString stringWithFormat:@"%@",_ShopModel.in_sum];
        activelab.text = [NSString stringWithFormat:@"参与团次：%d次",myactivenum.intValue];
        
        activelab.font = [UIFont systemFontOfSize:ZOOM6(24)];
        
        NSMutableAttributedString *activeStr ;
        if(activelab.text && myactivenum)
        {
            activeStr = [[NSMutableAttributedString alloc]initWithString:activelab.text];
            [activeStr addAttribute:NSForegroundColorAttributeName value: tarbarrossred range:NSMakeRange(5, myactivenum.length)];
            [activelab setAttributedText:activeStr];
        }
        
        UILabel *timelab = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(headimage.frame)+ZOOM6(20), CGRectGetMaxY(activelab.frame)-ZOOM6(10), ZOOM6(400), ZOOM6(40))];
        ...
        NSString *count = [NSString stringWithFormat:@"%@",_ShopModel.my_num];
        NSArray *mynum = [_myself_incode componentsSeparatedByString:@","];
        tisilab.text = [NSString stringWithFormat:@"您参与了:%zd团次",mynum.count];
        
        NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
        NSString *userid = [user objectForKey:USER_ID];
        if(userid.intValue == self.recordsModel.in_uid.intValue)
        {
            tisilab.text = @"恭喜你，中奖了！";
        }else{
            NSMutableAttributedString *noteStr1 ;
            if(tisilab.text)
            {
...
-(void)creatSectionView
{
    ...
    NSArray *titleArr=@[@"拼团详情",@"参与记录",@"图文详情"];
...
-(void)creatBigTableview
{
    ...
    [self.MyBigtableview registerClass:[MyTreasureGroupsShareCell class] forCellReuseIdentifier:@"cell"];
    
    [self.MyBigtableview registerNib:[UINib nibWithNibName:@"FightIndianaHeadTableViewCell" bundle:nil] forCellReuseIdentifier:@"FightIndianaHeadCell"];
}
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView
{
    if (tableView!=self.searchTableView) {
        ...
        {
            return self.fightIndianaData.count+1;
        }
        return 1;
    } else
...
{
    if (tableView!=self.searchTableView) {
        if(self.slectbtn.tag == 1000 && section>0)
        {
            UIView *view = [[UIView alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, ZOOM6(100))];
            view.backgroundColor = [UIColor whiteColor];
            UILabel *topLabel = [[UILabel alloc]init];
            topLabel.textColor = kMainTitleColor;
            topLabel.textAlignment = NSTextAlignmentCenter;
            topLabel.font = kFont6px(28);
            UILabel *num = [[UILabel alloc]init];
            num.textColor = kMainTitleColor;
            num.textAlignment = NSTextAlignmentCenter;
            num.font = kFont6px(24);
            ...
            TreasureGroupsModel *model = self.fightIndianaData[section-1];
            
            NSString *colorStr = [NSString stringWithFormat:@"%zd",section];
            NSString * issue_code = [NSString stringWithFormat:@"%@",model.issue_code];
            NSString *str = [NSString stringWithFormat:@"第%@团，%@",colorStr,[issue_code containsString:@"null"]||[issue_code isEqualToString:@"0"]?@"未满":@"满员"];
            [topLabel setAttributedText:[NSString getOneColorInLabel:str ColorString:colorStr Color:tarbarrossred font:kFont6px(28)]];
            
            num.text = [issue_code containsString:@"null"]||[issue_code isEqualToString:@"0"] ? @"" : [NSString stringWithFormat:@"（幸运号码：%@）",model.u_code];
            return view;
...
-(CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section
{
    if (tableView!=self.searchTableView) {
        if(self.slectbtn.tag==1000 && section >0)
        {
            TreasureGroupsModel *model = self.fightIndianaData[section-1];
            NSString * issue_code = [NSString stringWithFormat:@"%@",model.issue_code];
            return [issue_code containsString:@"null"]||[issue_code isEqualToString:@"0"] ? ZOOM6(130) : ZOOM6(180) ;
        }else
            return 40;
    } else
        return ZOOM(80);
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView!=self.searchTableView) {
        ...
        else
        {
            ...
            {
                //                self.fightIndianaData.count?(heigh = ZOOM6(940)):(heigh=0.01);
                heigh = ZOOM6(940);
            }else{
                
                TreasureGroupsModel *model = self.fightIndianaData[indexPath.section-1];
                
                NSInteger count = (model.user.count + 1) < _ShopModel.group_number.integerValue ? _ShopModel.group_number.integerValue : model.user.count + 1;
                
                heigh= ZOOM6(150)*(count/5+(count%5?1:0));//ZOOM6(20)*(count/5)+
    ...
    if (tableView!=self.searchTableView) {
        ...
        }else if (self.slectbtn.tag==1001)
        {
            ...
            if(indexPath.row ==0 )
            {
                ...
                {
                    FightIndianaRecordModel *model = _participateArray[indexPath.row-1];
                    cell.isFightIndiana = YES;
                    [cell refreshIndiana:model];
                }
                return cell;
            }
        ...
        } else if(self.slectbtn.tag == 1000)
        {
            ...
            if(indexPath.section == 0)
            {
                FightIndianaHeadTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"FightIndianaHeadCell"];
                if(!cell)
                {
                    cell = [[FightIndianaHeadTableViewCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:@"FightIndianaHeadCell"];
                }
                ...
                cell.selectionStyle = UITableViewCellSelectionStyleNone;
                [cell refresh:_ShopModel Group:group_model];
                return cell;
                ...
                TreasureGroupsModel *model = self.fightIndianaData[indexPath.section-1];
                [cell loadData:model needNum:_ShopModel.group_number.integerValue shopName:_ShopModel.shop_name];
                return cell;
            }
            return nil;
        }
...
-(void)butclick:(UIButton*)sender
{
    ...
    if(self.slectbtn.tag==1002)
    {
        self.MyBigtableview.frame = CGRectMake(0, viewHeigh, kApplicationWidth, kApplicationHeight+kUnderStatusBarStartY-viewHeigh);
        self.MyBigtableview.tableFooterView=_tableFootView;
    }
    else if (self.slectbtn.tag==1001)
    {
        ...
        CGFloat spaceHeigh = 0;
        if(self.fightIndianaData.count)
        {
            spaceHeigh = ZOOM6(100);
        }else{
            spaceHeigh = kApplicationHeight-Height_NavBar-kUnderStatusBarStartY-55-ZOOM6(940);
        }
        _footView=[[UIView alloc]initWithFrame:CGRectMake(0, 0, kApplicationWidth, spaceHeigh)];
        self.MyBigtableview.frame = CGRectMake(0, viewHeigh, kApplicationWidth, kApplicationHeight+kUnderStatusBarStartY-55-viewHeigh);
        self.MyBigtableview.tableFooterView= _footView;
...
-(void)contactClick:(UIButton*)sender
{
    ...
    MyTreasureGroupsShareVC *groupVC = [[MyTreasureGroupsShareVC alloc]init];
    groupVC.model = _ShopModel;
    groupVC.shareImgUrl = _newimage;
...
#pragma 获取随机商品链接
-(void)markrequestShopLinkHttpsuccess:(void(^)())success
{
    AFHTTPRequestOperationManager *manager=[AFHTTPRequestOperationManager manager];
    NSMutableDictionary *paraments=[NSMutableDictionary dictionary];
    ...
    manager.requestSerializer.timeoutInterval = 10;
    NSString *url = [NSString stringWithFormat:@"%@shop/shareShop?token=%@&version=%@&realm=%@&getShop=true&hobby=20",[NSObject baseURLStr], token,VERSION, realm];;
    ...
    [manager POST:URL parameters:paraments success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if (responseObject!=nil) {
            ...
            if(statu.intValue==1)//请求成功
            {
                indianaShare_link = responseObject[@"link"];
                if(indianaShare_link != nil)
                {
                    if(success)
                    {
                        success();
...
- (void)shopRequest:(int)tag Sharelink:(NSString*)shareLink success:(void(^)())success
{
    AFHTTPRequestOperationManager *manager=[AFHTTPRequestOperationManager manager];
    NSMutableDictionary *paraments=[NSMutableDictionary dictionary];
    ...
    NSString *url=url=[NSString stringWithFormat:@"%@shop/getShopLink?version=%@&shop_code=%@&realm=%@&token=%@&share=%@&getShop=true&indiana=1",[NSObject baseURLStr],VERSION,_ShopModel.shop_code,realm,token,@"2"];;
    NSString *URL=[MyMD5 authkey:url];
    ...
    [manager POST:URL parameters:paraments success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if (responseObject!=nil) {
            ...
            if(str.intValue==1)
            {
                ...
                if(shareLink != nil)
                {
                    [userdefaul setObject:[NSString stringWithFormat:@"%@",shareLink] forKey:QR_LINK];
                }
                else if(_shareShopurl)
                {
...
- (void)httpGetTixianShare
{
    ...
    if (data) {
        ...
        if (responseObject!=nil) {
            ...
            NSString *taskValue = [Signmanager SignManarer].indianaShareRaward;
            NSArray *valueArray = [taskValue componentsSeparatedByString:@","];
            NSString *sharLink = @"";
            ...
            if([sharLink isEqualToString:@"share=spellGroup"])//拼团
            {
                ...
                [TaskSignModel IndiaHttpR_code:^(id data) {//获取拼团团号
                    ...
                    if(model.data != nil && model.status == 1){
                        indianaShare_link = [NSString stringWithFormat:@"%@view/activity/pack.html?realm=%@&r_code=%@",[NSObject baseURLStr_H5],[user objectForKey:USER_ID],model.data];
                    }else{
                        
                        indianaShare_link = [NSString stringWithFormat:@"%@view/activity/pack.html?realm=%@",[NSObject baseURLStr_H5],[user objectForKey:USER_ID]];
                    }
                    
                    [weakself shopRequest:9000 Sharelink:indianaShare_link success:^{
            ...
            {
                indianaShare_link = [NSString stringWithFormat:@"%@view/activity/mission.html?realm=%@",[NSObject baseURLStr_H5],[user objectForKey:USER_ID]];
                [self shopRequest:9000 Sharelink:indianaShare_link success:^{
                    [extrabonusview shareData];
                }];
                
            }else if ([sharLink isEqualToString:@"share=indiana"])//1分夺宝
            {
                [self shopRequest:9000 Sharelink:nil success:^{
                    [extrabonusview shareData];
                }];
            }else if([sharLink isEqualToString:@"share=link"]){//随机商品分享
                kWeakSelf(self);
                [weakself markrequestShopLinkHttpsuccess:^{
                    [weakself shopRequest:9000 Sharelink:indianaShare_link success:^{//获取文案
                        [extrabonusview shareData];
                    }];
                }];
            }
...
-(void)shareClick:(UIButton*)sender
{
    ...
    if(_shareModelview)
    {
        ...
        [UIView animateWithDuration:0.5 animations:^{
            ...
            //获取商品链接
            [self shopRequest:tag Sharelink:nil success:^{
...
-(void)gotoshare:(int)sharetag success:(void(^)())success
{
    ...
    }else if (sharetag==9001)//微信朋友圈
    {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            UIImage *newimg = [pi getImage:shopimage withQRCodeImage:qrpicimage withText:@"Indiandetail" withPrice:shopprice WithTitle:nil];
            MyLog(@"newimg = %@",newimg);
...
- (void)Recordclick:(UIButton*)sender
{
    ...
    tiVC.type = IndianaRecords_TreasureGrop;
...
{
    TFIndianaRecordViewController *tiVC = [[TFIndianaRecordViewController alloc] init];
    tiVC.type = IndianaAnnounce_TreasureGrop;
    tiVC.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:tiVC animated:YES];
}
...
{
    extrabonusview = [[IndianaPopView alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight) Price:self.shop_se_price];
    __weak IndianaPopView *view = extrabonusview;
    view.tapHideMindBlock = ^{
        [view remindViewHiden];
    };
    ...
    view.confirmOrderBlock = ^(NSString* price,NSString *num,NSString *ReductionPrice){
        ...
        [weakself togoAffirm:price Num:num ReductionPrice:ReductionPrice];
    };
...
- (NSMutableArray *)fightIndianaData
{
    if(_fightIndianaData == nil)
    {
        _fightIndianaData = [NSMutableArray array];
    }
    return _fightIndianaData;
}
#pragma mark 普通分享失败
