...
@interface  MJRefreshBaseView()
{
    ...
    __weak UIActivityIndicatorView *_activityView1;
...
//        UIImageView *arrowImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"加载1.png"]];
...
//        imgView.center = CGPointMake(kScreenWidth/2, kApplicationHeight/2);
        imgView.tag = 778;

        NSMutableArray *anArr = [NSMutableArray array];
        
        for (int i = 0 ; i<15; i++) {
            NSString *gStr = [NSString stringWithFormat:@"加载%d",i+1];
        ...
        imgView.animationImages = anArr;    //动画图片数组
        imgView.animationDuration = 0.5;      //执行一次完整动画所需的时长
        imgView.animationRepeatCount = 0;   //无限

        
        [self addSubview:_activityView = (UIActivityIndicatorView *)imgView];
    }
...
//                imgView.center = CGPointMake(kScreenWidth/2, kApplicationHeight);
        imgView.tag = 778;
        ...
        imgView.animationImages = anArr;    //动画图片数组
        imgView.animationDuration = 1;      //执行一次完整动画所需的时长
        imgView.animationRepeatCount = 0;   //无限
        
        
        [self addSubview:_activityView1 = activityView];
    }
...
- (void)layoutSubviews
{
    ...
    self.activityView1.center = self.arrowImage.center;
}
...
//    MyLog(@"self.state: %d, state: %d", self.state, state);
    // 0.存储当前的contentInset
    if (self.state != MJRefreshStateRefreshing) {
...
- (void)setState1:(MJRefreshState)state
{
    // 0.存储当前的contentInset
    if (self.state != MJRefreshStateRefreshing) {
        _scrollViewOriginalInset = self.scrollView.contentInset;
        ...
        case MJRefreshStateNormal: // 普通状态
        {
            if (self.state == MJRefreshStateRefreshing) {
                [UIView animateWithDuration:MJRefreshSlowAnimationDuration animations:^{
                    self.activityView1.alpha = 0.0;
                } completion:^(BOOL finished) {
                    // 停止转圈圈
                    [self.activityView1 stopAnimating];
                    
                    // 恢复alpha
                    self.activityView1.alpha = 1.0;
                }];
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(MJRefreshSlowAnimationDuration * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    // 再次设置回normal
                    _state = MJRefreshStatePulling;
