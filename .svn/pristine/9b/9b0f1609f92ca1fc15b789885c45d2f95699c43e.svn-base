...
- (NSInteger)sum:(Class)modelClass variable:(NSString *)variable where:(id)where, ...
{
    NSString *tableName = [modelClass getTableName];
    LKDBCheck_tableNameIsInvalid(tableName);
    NSMutableString* rowCountSql = [NSMutableString stringWithFormat:@"select sum(%@) as sumvalue from %@",variable, tableName];
