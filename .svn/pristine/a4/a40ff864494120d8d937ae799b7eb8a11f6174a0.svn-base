...
@interface TFSlidePageScrollView ()<UIScrollViewDelegate,TFCustomTitleViewDelegate>{
    struct {
...
@implementation TFSlidePageScrollView
...
- (void)setTyDelegate:(id<TFSlidePageScrollViewDelegate>)tyDelegate
{
...
- (void)updateHeaderContentView
{
    ...
    if (_pageTabBar) {
        _pageTabBar.tfCustomTitleViewDelegate = self;
...
{
//    MyLog(@"_headerContentView: %@", _headerContentView);
    
    if (index < 0 || index >= _pageViewArray.count || _headerContentView.frame.origin.y> 20) {
        MyLog(@"scrollToPageIndex index illegal");
        return NO;
    }
...
- (void)pageScrollViewDidScroll:(UIScrollView *)pageScrollView changeOtherPageViews:(BOOL)isNeedChange
{
    ...
    //    //curr = %d, y = %f, headerContentViewheight = %f, pageTabBarHieght = %f", (int)_curPageIndex,offsetY, headerContentViewheight, pageTabBarHieght);
    ...
    if ((int)offsetY <=(int) (-headerContentViewheight)) {
        ...
        if (!CGRectEqualToRect(_headerContentView.frame, frame)) {
            ...
            if (_delegateFlags.pageTabBarScrollOffset) {
                [_tyDelegate slidePageScrollView:self pageTabBarScrollOffset:offsetY state:TFPageTabBarStateStopOnButtom];
            }
        }
        ...
        if (_delegateFlags.pageTabBarScrollOffset) {
            [_tyDelegate slidePageScrollView:self pageTabBarScrollOffset:offsetY state:TFPageTabBarStateScrolling];
        }
        ...
        if (!CGRectEqualToRect(_headerContentView.frame, frame)) {
            ...
            if (_delegateFlags.pageTabBarScrollOffset) {
                [_tyDelegate slidePageScrollView:self pageTabBarScrollOffset:offsetY state:TFPageTabBarStateStopOnTop];
            }
        }
...
{
    return [self scrollToPageIndex:index animated:NO];
}
