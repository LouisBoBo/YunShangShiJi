...
+ (MB_INSTANCETYPE)showHUDAddTo:(UIView *)view animated:(BOOL)animated {
    MBProgressHUD *hud = [[self alloc] initWithView:view];
    hud.mode=MBProgressWTF;
    if(hud)
    {
        [view addSubview:hud];
    }
...
- (void)updateIndicators {
	...
	if (mode == MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
		...
		[indicator removeFromSuperview];
		self.indicator = nil;
    }else if (mode == MBProgressWTF ) {
        [indicator removeFromSuperview];
        self.opacity=0.0f;
        self.indicator = [self indicatorView];
        [(UIActivityIndicatorView *)indicator startAnimating];
        [self addSubview:indicator];
    }
}
...
{
    UIImageView *iv = [[UIImageView alloc] initWithFrame:CGRectMake(kScreenWidth/2-15, kScreenWidth/2-47, 30, 30)];
