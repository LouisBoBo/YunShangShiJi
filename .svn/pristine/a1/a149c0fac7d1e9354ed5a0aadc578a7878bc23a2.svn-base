...
- (void)viewDidLoad
{
    ...
    [self initBottomMenu];
    [self initControls];
//    [self.view SetStatues];
    
    UINib *nib = [UINib nibWithNibName:@"DoPhotoCell" bundle:nil];
    [_cvPhotoList registerNib:nib forCellWithReuseIdentifier:@"DoPhotoCell"];
    _cvPhotoList.frame=CGRectMake(0, kUnderStatusBarStartY, self.view.frame.size.width, self.view.frame.size.height-kUnderStatusBarStartY);
...
- (void)initControls
{
    ...
    _btUp.frame=Frame(FrameX(_btDown), kUnderStatusBarStartY+kApplicationWidth-FrameXW(_btUp), FrameW(_btDown), FrameH(_btDown));
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        CGFloat width = (kScreenWidth - 15)/4;
