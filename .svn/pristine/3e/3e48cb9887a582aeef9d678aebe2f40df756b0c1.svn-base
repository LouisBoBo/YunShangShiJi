...
- (void) setLongString:(NSString *)str font:(UIFont *)font withFitWidth:(CGFloat)width maxHeight:(CGFloat)maxHeight{
    self.numberOfLines = 4;
    CGSize resultSize = [str getSizeWithFont:font constrainedToSize:CGSizeMake(width, maxHeight)];
    CGFloat resultHeight = resultSize.height;
//    if (maxHeight > 0 && resultHeight > maxHeight) {
//        NSMutableString *newstr = [NSMutableString stringWithString:[str substringToIndex:str.length-6]];
//        str = [NSString stringWithFormat:@"%@ 查看全文",newstr];
//    }
    CGRect frame = self.frame;
    frame.size.height = resultHeight;
