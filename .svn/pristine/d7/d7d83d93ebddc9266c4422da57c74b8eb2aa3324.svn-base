...
-(instancetype)initWithFrame:(CGRect)frame withViews:(NSArray *)views withButtonNames:(NSArray *)names withThreeAnimation:(int)choose withTag:(int)tag withImage:(NSArray*)images{
...
+(instancetype)scrollWithFrame:(CGRect)frame withViews:(NSArray *)views withButtonNames:(NSArray *)names withThreeAnimation:(int)choose withTag:(int)tag withImage:(NSArray*)images{
    return [[self alloc]initWithFrame:frame withViews:views withButtonNames:names withThreeAnimation:choose withTag:tag withImage:images];
}
...
-(void)addScroll1{
    ...
    if (self.xl_buttonNames.count <=5) {
        ...
        self.scroll1.contentSize =CGSizeMake(screen_width/4*self.xl_buttonNames.count, 0);
    }
    ...
    for (int i =0; i<self.xl_buttonNames.count; i++) {
        ...
        if (self.xl_buttonNames.count <=4) {
            ...
            temp.frame =CGRectMake(screen_width/4*i, 0, screen_width/4, self.xl_topHeight?self.xl_topHeight:50);
        }
        ...
        NSArray *images=[self.xl_buttonImages[i] componentsSeparatedByString:@","];
        
        UIImageView *imageview=[[UIImageView alloc]initWithFrame:CGRectMake(2, (40-20)/2, 20, 20)];
        
        if(images.count)
        {
            [imageview sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",[NSObject baseURLStr_Upy],images[0]]]];
        }
    ...
    if (self.xl_isSliderCorner) {
        [self.view2.layer setCornerRadius:self.xl_sliderCorner?self.xl_sliderCorner:4];
    }
...
//     self.scroll2 =[[UIScrollView alloc]initWithFrame:CGRectMake(0, 30, screen_width, self.frame.size.height -(self.xl_topHeight?self.xl_topHeight:50))];
    ...
    for (int i =0; i<self.xl_buttonNames.count; i++) {
        ...
        if (1000==1000+i) {//self.tag+1
            
               self.scroll2.contentOffset = CGPointMake(320*(i) ,0);
...
-(void)changed:(UIButton *)button{
    ...
    UIButton *clickbut=(UIButton*)[self.scroll1 viewWithTag:button.tag];

    NSString *str=[NSString stringWithFormat:@"%d",(clickbut.tag%1000)];
    NSNotification *gundongnote=[[NSNotification alloc]initWithName:@"gundong" object:str userInfo:nil];
    [[NSNotificationCenter defaultCenter]postNotification:gundongnote];
    ...
    UIButton *temp=(UIButton*)[self.scroll1 viewWithTag:self.scroll1.tag];
    ...
    if (self.xl_buttonNames.count <=4) {
        ...
        self.scroll2.contentOffset =CGPointMake(button.center.x*4 -screen_width/2, 0);
    }
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (self.xl_isMoveButton) {
        if (self.xl_buttonNames.count <=4) {
            ...
            button.transform =CGAffineTransformMakeTranslation((scrollView.contentOffset.x -button.frame.size.width*4*_x)/4/3, 0);
            self.view2.transform =CGAffineTransformMakeTranslation(scrollView.contentOffset.x/4, 0);
        }
    }else {
        if (self.xl_buttonNames.count <=4) {
            self.view2.transform =CGAffineTransformMakeTranslation(scrollView.contentOffset.x/self.xl_buttonNames.count, 0);
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    for (UIButton *temp in self.buttons) {
        ...
        if (self.xl_buttonNames.count <=4) {
            ...
            x2 =scrollView.contentOffset.x/4;
        }
       ...
       UIButton *button =self.buttons[_x];
        ...
        if (_x<xAfter) {
            ...
            if (_x>=2 && _x<=self.xl_buttonNames.count-4 ) {
                [UIView animateWithDuration:0.2 animations:^{
                    self.scroll1.contentOffset =CGPointMake(button.center.x -screen_width/4*1.5, 0);
                }];
            }
            ...
            if (_x>=3 && _x<=self.xl_buttonNames.count-2 ) {
                [UIView animateWithDuration:0.2 animations:^{
                    self.scroll1.contentOffset =CGPointMake(button.center.x -screen_width/4*3.5, 0);
                }];
            }
            if ((self.buttons.count ==6)|7 &&_x<3) {
                if ((_x ==1)|2) {
    ...
    NSString *str=[NSString stringWithFormat:@"%d",_x];
    NSNotification *gundongnote=[[NSNotification alloc]initWithName:@"gundong" object:str userInfo:nil];
    [[NSNotificationCenter defaultCenter]postNotification:gundongnote];
