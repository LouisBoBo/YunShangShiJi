...
- (void)panGestureHandle:(UIPanGestureRecognizer *)gesture {
    ...
    if (gesture.state == UIGestureRecognizerStateChanged) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(draggableContainer:draggableDirection:widthRatio:heightRatio:)]) {
            ...
            //手势滑动
            if(self.delegate && [self.delegate respondsToSelector:@selector(panGesturedraggaChangebleContainer:cardView:didSelectIndex:)])
            {
                [self.delegate panGesturedraggaChangebleContainer:self cardView:(CCDraggableCardView *)gesture.view didSelectIndex:gesture.view.tag];
            }
    ...
    if (gesture.state == UIGestureRecognizerStateEnded ||
        ...
        //手势结束 且视图消失
        if(gesture.state == UIGestureRecognizerStateEnded && fabs(widthRatio) > kBoundaryRatio)
        {
            if(self.delegate && [self.delegate respondsToSelector:@selector(panGesturedraggableContainer:cardView:didSelectIndex:)])
            {
                [self.delegate panGesturedraggableContainer:self cardView:(CCDraggableCardView *)gesture.view didSelectIndex:gesture.view.tag];
            }
        }
    }
}
...
- (void)removeFormDirection:(CCDraggableDirection)direction {
    ...
    if (self.moving) {
        ...
        CustomCardView *firstView = [self.currentCards firstObject];
        
        //手势滑动
        if(self.delegate && [self.delegate respondsToSelector:@selector(panGesturedraggaChangebleContainer:cardView:didSelectIndex:)])
        {
            self.direction = direction;
            [self.delegate panGesturedraggaChangebleContainer:self cardView:firstView didSelectIndex:firstView.tag];
        }
        ...
        {
            self.direction = direction;
            [self.delegate panGesturedraggableContainer:self cardView:firstView didSelectIndex:firstView.tag];
        }
