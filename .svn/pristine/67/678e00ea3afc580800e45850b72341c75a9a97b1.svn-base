...
//        [button setTitleColor:BarBtnSelectedColor forState:UIControlStateSelected];
//        [button setTitleColor:BarBtnSelectedColor forState:UIControlStateHighlighted];
          if (idx!=0) {
              UIView *line=[[UIView alloc]initWithFrame:CGRectMake(0, 5, 0.5, CGRectGetHeight(self.bounds)-10)];
              line.backgroundColor=[UIColor colorWithRed:220/255.0 green:220/255.0 blue:220/255.0 alpha:1.0];
              [button addSubview:line];
          }
//          button.redBadge.hidden=NO;
        [_containerView addSubview:button];
        [buttons addObject:button];
      }
    }];
    [_indicatorLayer setBackgroundColor:[BarBtnSelectedColor CGColor]];
    [self setNeedsLayout];
