...
//  YunShangShiJi
//
//  Created by jingaiweiyi on 2016/11/12.
//  Copyright © 2016年 ios-1. All rights reserved.
//

#import "YiFuBaseDBModel.h"
@implementation YiFuBaseDBModel
...
+(LKDBHelper *)getUsingLKDBHelper
{
    ...
    dispatch_once(&onceToken, ^{
        NSString *sqlitePath = [YiFuBaseDBModel downloadPath];
        NSString* dbpath = [sqlitePath stringByAppendingPathComponent:[NSString stringWithFormat:@"YiFuDefault.db"]];
...
+ (NSString *)downloadPath {
    ...
    NSString *downloadPath = [documentPath stringByAppendingPathComponent:@"YiFuDB"];
    NSLog(@"%@",downloadPath);
    return downloadPath;
}
...
+(NSString *)getCreateTableSQL
{
    ...
    NSLog(@"createTableSQL: %@", createTableSQL);
