...
#define kXSpace 0.0                              /** 距离父视图边界横向最小距离 */
#define kYSpace 0.0                              /** 距离俯视图边界纵向最小距离 */
#define kTagHorizontalSpace 28.0                 /** 标签左右空余距离 */
#define kTagVerticalSpace 20.0                   /** 标签上下空余距离 */
#define kShopTagHeight 18.0                   /** 标签下方商品标签距离 */
#define kShopTagWidth 60.0
#define kPointWidth 0.0                          /** 白点直径 */
#define kPointSpace 0.0                          /** 白点和阴影尖角距离 */
#define kAngleLength 0//(self.zy_height / 2.0 - 2)  /** 黑色阴影尖交宽度 */
...
/** 商品标签 */
@property (nonatomic, weak) UIImageView *shopTagImg;
...
//    backLayer.fillColor = [[UIColor clearColor] colorWithAlphaComponent:.7].CGColor;
//    backLayer.shadowOffset = CGSizeMake(0, 2);
//    backLayer.shadowColor = [UIColor clearColor].CGColor;
...
//    self.pointLayer = pointLayer;
    ...
    if (self.tagInfo.isShopTag) {
        UIImageView *shopTagImg=[[UIImageView alloc]init];
        shopTagImg.image=[UIImage imageNamed:@"icon_shop_buy"];
        shopTagImg.contentMode=UIViewContentModeScaleAspectFit;
        shopTagImg.userInteractionEnabled=YES;
        [self addSubview:shopTagImg];
        self.shopTagImg=shopTagImg;
    }
...
- (void)setupGesture
{
    ...
    [self.shopTagImg addGestureRecognizer:tap2];
    ...
    [self.titleLabel addGestureRecognizer:lop];

    UIPanGestureRecognizer *pan = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(handlePanGesture:)];
    [self.titleLabel addGestureRecognizer:pan];
}
...
//    self.titleLabel.zy_height += kTagVerticalSpace;
    ...
    self.titleLabel.titleLabel.font = [UIFont systemFontOfSize:ZOOM6(28)];
...
//    [self.titleLabel sizeToFit];
    ...
    CGSize titleSize = [title sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:ZOOM6(28)]}];
    self.titleLabel.zy_width = kTagHorizontalSpace + titleSize.width;
    self.titleLabel.zy_height = kTagVerticalSpace + titleSize.height;
    ...
    if (state == ZYTagViewStateArrowLeft) {
        ...
        if (self.zy_x < self.zy_width) {
            self.zy_x = self.zy_width;
        }
        if (self.zy_x > superview.zy_width - kXSpace - self.zy_width) {
            self.zy_x = superview.zy_width - kXSpace - self.zy_width;
        }
...
- (void)layoutSubviewsWithState:(ZYTagViewState)state arrowPoint:(CGPoint)arrowPoint
{
    ...
    if (self.tagInfo.isShopTag) {
        self.zy_height += kShopTagHeight;
    }
//    self.zy_centerY = arrowPoint.y;
    self.zy_y = arrowPoint.y;

    self.titleLabel.zy_y = 0;
    NSLog(@"self.zy_height--%f self.titleLabel.zy_height %f",self.zy_height,self.titleLabel.zy_height);
   ...
   // self.backgroundColor=[UIColor whiteColor];
    self.titleLabel.titleEdgeInsets=UIEdgeInsetsMake(0, kBackCornerRadius, 0, 0);
    if (state == ZYTagViewStateArrowLeft || state == ZYTagViewStateArrowLeftWithDelete) {
        ...
        // 标签
        self.titleLabel.zy_x = kPointWidth;
//        self.titleLabel.zy_x = kPointWidth + kAngleLength;
        self.shopTagImg.frame=CGRectMake(self.zy_width-kShopTagWidth, self.titleLabel.height-4, kShopTagWidth, kShopTagHeight);
        ...
        [self.titleLabel setBackgroundImage:[UIImage imageNamed:@"topic_biaoqian"] forState:UIControlStateNormal];
    }else if(state == ZYTagViewStateArrowRight || state == ZYTagViewStateArrowRightWithDelete) {
        // 根据字调整控件大小
//        self.zy_right = arrowPoint.x + kPointWidth / 2.0;
        self.zy_x = arrowPoint.x;
        // 背景
        ...
        if (state == ZYTagViewStateArrowRight) {
            ...
            self.shopTagImg.frame=CGRectMake(0, self.titleLabel.height-4, kShopTagWidth, kShopTagHeight);
        ...
        [self.titleLabel setBackgroundImage:[UIImage imageNamed:@"topic_rightbiaoqian"] forState:UIControlStateNormal];
    }
...
//            if (point.x<0) {
//                referencePoint.x=0;
//            }else if (point.x > self.superview.zy_width-self.zy_width)
//                referencePoint.x=self.superview.zy_width-self.zy_width;
//        }
...
//            if (point.x<0) {
//                referencePoint.x=self.width;
//            }else if (point.x > self.superview.zy_width)
//                referencePoint.x=self.superview.zy_width;
//        }
            break;
        default:
            break;
    ...
    if (referencePoint.x<0) {
        referencePoint.x=0;
    }else if (referencePoint.x > self.superview.zy_width-self.zy_width)
        referencePoint.x=self.superview.zy_width-self.zy_width;
    */
    if (self.state==ZYTagViewStateArrowLeft) {
        if (referencePoint.x < kXSpace + kPointWidth / 2.0) {
            referencePoint.x = kXSpace + kPointWidth / 2.0;
        }else if (referencePoint.x > self.superview.zy_width - self.zy_width - kXSpace - kPointWidth /2.0)
            referencePoint.x = self.superview.zy_width - self.zy_width - kXSpace - kPointWidth /2.0;
    }else if (self.state==ZYTagViewStateArrowRight) {
        if (referencePoint.x < self.zy_width+kXSpace + kPointWidth / 2.0) {
            referencePoint.x = self.zy_width+kXSpace + kPointWidth / 2.0;
...
//    NSLog(@"panPoint.x = %f panPoint.y = %f",self.arrowPoint.x,self.arrowPoint.y);
...
//        self.tagInfo.point = CGPointMake(self.zy_x + kPointWidth / 2, self.zy_y + self.zy_height / 2.0);
        self.tagInfo.point = CGPointMake(self.zy_x + kPointWidth / 2, self.zy_y );
...
//        self.tagInfo.point = CGPointMake(self.zy_right - kPointWidth / 2, self.zy_y + self.zy_height / 2.0);
    ...
    CGFloat x=_tagInfo.point.x/superview.frame.size.width;
    CGFloat y=_tagInfo.point.y/superview.frame.size.height;
    [self.tagInfo.dic setValue:[NSString stringWithFormat:@"%f",x] forKey:@"label_x"];
    [self.tagInfo.dic setValue:[NSString stringWithFormat:@"%f",y]  forKey:@"label_y"];
    int i = self.tagInfo.direction == ZYTagDirectionRight ? 1 : 0;
    NSString *direction=[NSString stringWithFormat:@"%zd",i];
    [self.tagInfo.dic setValue:direction  forKey:@"direction"];
    // 更新proportion
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)pan
{
    ...
    switch (pan.state) {
        ...
        {
//            CGFloat x=self.state==ZYTagViewStateArrowLeft?panPoint.x - self.panTmpPoint.x:panPoint.x + self.panTmpPoint.x;
            CGFloat x=panPoint.x - self.panTmpPoint.x;
            [self changeLocationWithGestureState:UIGestureRecognizerStateChanged
                                   locationPoint:CGPointMake(x, panPoint.y - self.panTmpPoint.y)];
        }
            break;
...
- (void)updateTagInfo:(ZYTagInfo *)tagInfo {
