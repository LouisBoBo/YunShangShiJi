...
@interface TopicdetailsViewController ()<UICollectionViewDelegate, UICollectionViewDataSource, StickyHeaderFlowLayoutDelegate,PopoverViewDelegate,CelldidselectDelegate>
...
{
    //导航条
    self.tabheadview=[[UIImageView alloc]initWithFrame:CGRectMake(0, 0, kApplicationWidth, 64)];
...
//    topic.theme_id = [NSString stringWithFormat:@"%@",self.theme_id];
...
{
    kSelfWeak;
    [TopicPublicModel LastComments:(NSString*)weakSelf.theme_id Page:weakSelf.currPage Pagesize:5 Success:^(id data) {
...
//            if(cmodel.pics.length>10 || cmodel.shop_list.count)
//            {
...
- (TopfooterView*)topfooterView
{
    if(!_topfooterView)
    {
        _topfooterView = [[TopfooterView alloc]initWithFrame:CGRectMake(0, 0,kScreenWidth, _footViewHeigh) Data:detailmodel.post_details];
        ...
        footviiew.shareBlock = ^(NSInteger tag)
        {
            if(tag == 3)
            {
                [__weakSelf creatShareView];
            }else{
                [__weakSelf goShare:tag+(tag==0?0:2)];
            }
        };
        ...
        footviiew.shopBlock = ^(NSInteger tag ,NSString *shopcode){
            ...
            if([shopcode isEqualToString:@"更多"])
            {
                ...
                commend.theme_id = __weakSelf.theme_id;
                [__weakSelf.navigationController pushViewController:commend animated:YES];
            }else{
                ...
                [__weakSelf.navigationController pushViewController:shopdetail animated:YES];
            }
        };
        ...
        footviiew.customTagBlock = ^(NSString *ID ,NSString *title,BOOL repeat){
            ...
            if(repeat)
            {
                ...
                {
                    [__weakSelf repeatcustomTagToVC:ID Title:title];
                }
            }
            else{
...
{
    if (!_collectionView) {
        _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 64, self.view.bounds.size.width, self.view.bounds.size.height-64-50) collectionViewLayout:[self layout]];
        ...
        [_collectionView registerClass:[CollectionHeaderView class] forSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:@"HeaderView"];

    }
    
    //下拉刷新
//    __weak TopicdetailsViewController *myController = self;
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section
{
    if(section == 0)
    {
        if(detailmodel.post_details.theme_type == 1)
        {
            return detailmodel.post_details.shop_list.count;
        }else{
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    if(indexPath.section == 0)
    {
        if(detailmodel.post_details.theme_type ==4 && indexPath.item == 0)
        {
            ...
            ESWeakSelf;
            cell.tagsBlock = ^(NSString *ID ,NSString *title,NSInteger labeltype){//标签
    ...
    {
        CommendCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"CommendCell" forIndexPath:indexPath];
        ESWeakSelf;
        cell.RelevantBlock = ^(NSInteger index)
        {
            IntimateCircleModel *model = __weakSelf.recommendData[index];
            [__weakSelf toTopic:model];
        };
        [cell refreshimageData:__weakSelf.recommendData isfresh:is_fresh];
        is_fresh = NO;
        return cell;
    }
    return nil;
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    if (kind == UICollectionElementKindSectionFooter) {
        ...
        if(indexPath.section == 0)
        {
            CollectionHeaderView *footer = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:@"HeaderView" forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView willDisplaySupplementaryView:(UICollectionReusableView *)view forElementKind:(NSString *)elementKind atIndexPath:(NSIndexPath *)indexPath NS_AVAILABLE_IOS(8_0);
{
    if (elementKind == UICollectionElementKindSectionFooter) {
        ...
        if(indexPath.section == 0 && !self.browseFlag && [self.comefrom isEqualToString:@"穿搭任务"])
        {
                ...
                if(mustLiulanCount > liulanNumber)//浏览多次
                {
                    ...
                    [TaskSignModel getTaskHttp:index_id Day:day Success:^(id data) {
                        ...
                        if(model.status == 1)
                        {
                            ...
                            [weakSelf setTaskPopMindView:Task_liulanFinish];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout referenceSizeForHeaderInSection:(NSInteger)section
{
    ...
    }else if (section == 2)
    {
        ...
        {
            return CGSizeMake(kScreenWidth, ZOOM6(100));
        }
        return CGSizeZero;
    }
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout referenceSizeForFooterInSection:(NSInteger)section
{
    ...
    }else if (section == 0)
    {
        if(detailmodel.post_details !=nil)
        {
            return CGSizeMake(kScreenWidth, _footViewHeigh);
        }
    }
    return CGSizeZero;
}
...
- (CGFloat)getFootViewHeigh
{
    ...
    CGFloat shopviewH =(detailmodel.post_details.shop_list.count>0 && detailmodel.post_details.theme_type !=1)?ZOOM6(230):ZOOM6(20);
...
- (void)tagToVC:(NSInteger)tag Title:(NSString*)title
{
    ...
    item.ID = model.ID;
    item.name = title;
    if([model.type isEqualToString:@"风格"])
    {
