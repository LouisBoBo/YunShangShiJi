...
@interface IndianaDetailViewController ()<DShareManagerDelegate,UISearchBarDelegate, TFScreeningBackgroundDelegate,WXApiDelegate,MiniShareManagerDelegate>
...
- (void)httpActivityRule
{
    ...
    [manager GET:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if (responseObject!=nil) {
            ...
            if(responseObject[@"dbgz"] != nil)
            {
                NSDictionary *dic2 = responseObject[@"dbgz"][@"text"];
                if(dic2 !=nil)
                {
                    if(dic2[@"t0"] != nil)
                    {
...
-(void)creatBIGView
{
    ...
    yifenImageView.image = [UIImage imageNamed:@"indiana_detail_抽奖"];
...
//        }
        ...
        if(incodes.count > 2)
        {
            tistext = [NSString stringWithFormat:@"参与号码：%@,%@...",incodes[0],incodes[1]];
        }else{
            tistext = [NSString stringWithFormat:@"参与号码：%@",_myself_incode];
        }
...
-(void)creatSectionView
{
    ...
    NSArray *titleArr=@[@"图文详情",@"参与记录",@"抽奖规则"];
...
-(void)creatFootView
{
    ...
    NSString *str = _countTrea >0?@"立即参与":@"一分钱抽奖";
    ...
    if (tableView!=self.searchTableView) {
        ...
        }else if (self.slectbtn.tag==1001)
        {
            ...
            if(indexPath.row ==0 )
            {
                ...
                //                cell.data = manPicModel;
                //                NSInteger num = [_ShopModel.num integerValue];
                //                NSInteger vnum = self.virtual_num.integerValue;
                //                NSString *numStr = [NSString stringWithFormat:@"%ld", num + vnum];
                //                NSString *str = [NSString stringWithFormat:@"%@人次", numStr];
                //                cell.sharelable.attributedText = [NSString getOneColorInLabel:str ColorString:numStr Color:tarbarrossred fontSize:ZOOM(43)];
...
-(void)markrequestShopLinkHttpsuccess:(void(^)())success
{
    ...
    [manager POST:URL parameters:paraments success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if (responseObject!=nil) {
            ...
            if(statu.intValue==1)//请求成功
            {
                NSString *shop_se_price = [NSString stringWithFormat:@"%@",responseObject[@"shop"][@"shop_se_price"]];
                NSString *shop_brand = [NSString stringWithFormat:@"%@",responseObject[@"shop"][@"supp_label"]];
                NSString *discription = [NSString stringWithFormat:@"%@",responseObject[@"shop"][@"shop_name"]];
                indianaShare_link = responseObject[@"link"];
                
                if(![shop_se_price isEqualToString:@"(null)"] && ![shop_se_price isEqual:[NSNull null]])
                {
                    [userdefaul setObject:shop_se_price forKey:SHOP_PRICE];
                }
                if(![shop_brand isEqualToString:@"(null)"] && ![shop_brand isEqual:[NSNull null]])
                {
                    [userdefaul setObject:shop_brand forKey:SHOP_BRAND];
                }
                if(![discription isEqualToString:@"(null)"] && ![discription isEqual:[NSNull null]])
                {
                    [userdefaul setObject:discription forKey:DISCRIPTION];
                }
                if(![indianaShare_link isEqualToString:@"(null)"] && ![indianaShare_link isEqual:[NSNull null]])
                {
                    [userdefaul setObject:indianaShare_link forKey:QR_LINK];
                }
                ...
                if(shopdic !=NULL || shopdic!=nil)
                {
                    if(shopdic[@"four_pic"])
                    {
                        ...
                        [userdefaul setObject:[NSMutableString stringWithString:shopdic[@"shop_code"]] forKey:SHOP_CODE];
                    }
                }
                ...
                {
                    kWeakSelf(self);
                    [TypeShareModel getTypeCodeWithShop_code:shopdic[@"shop_code"] success:^(TypeShareModel *data) {
                        
                        if(data.status == 1 && data.type2 != nil)
                        {
                            [userdefaul setObject:[NSString stringWithFormat:@"%@",data.type2] forKey:SHOP_TYPE2];
                           [weakself taskrawardHttp:@"wxdddfx"];
                        }
            ...
            if(str.intValue==1)
            {
                ...
                if(shopdic !=NULL || shopdic!=nil)
                {
                    ...
                    if(shopdic[@"def_pic"])
                    {
                        ...
                        NSArray *imageArray = [shopdic[@"def_pic"] componentsSeparatedByString:@","];
        ...
        if (responseObject!=nil) {
            ...
            if([sharLink isEqualToString:@"share=spellGroup"])//拼团
            {
                ...
                [TaskSignModel IndiaHttpR_code:^(id data) {//获取拼团团号
                    ...
                    [[NSUserDefaults standardUserDefaults] setObject:[NSString stringWithFormat:@"%@",indianaShare_link] forKey:QR_LINK];
                    
                    [self taskrawardHttp:@"spellGroup_yf"];
                }];
            ...
            }else if ([sharLink isEqualToString:@"share=h5money"])//H5赚钱任务
            {
                ...
                [[NSUserDefaults standardUserDefaults] setObject:[NSString stringWithFormat:@"%@",indianaShare_link] forKey:QR_LINK];
                [self taskrawardHttp:@"h5money_yf"];
            }else if ([sharLink isEqualToString:@"share=indiana"])//1分夺宝
            {
                [self taskrawardHttp:@"indiana_yf"];
...
- (void)taskrawardHttp:(NSString*)strtype
{
    NSString *URL= [NSString stringWithFormat:@"%@paperwork/paperwork.json",[NSObject baseURLStr_Upy]];
    NSURL *httpUrl=[NSURL URLWithString:URL];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:httpUrl];
    ...
    if (data) {
        ...
        if (responseObject!=nil) {
            NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
            if (responseObject[strtype] != nil ){
                
                if([strtype isEqualToString:@"wxdddfx"])
                {
                    NSString *str0 = responseObject[@"wxdddfx"][@"title"];
                    NSString *price = [[NSUserDefaults standardUserDefaults]objectForKey:SHOP_PRICE];
                    NSString *brand = [[NSUserDefaults standardUserDefaults]objectForKey:SHOP_BRAND];
                    
                    str0 = [str0 stringByReplacingOccurrencesOfString:@"${replace}元" withString:[NSString stringWithFormat:@"%.1f元",price.floatValue*0.5]];
                    str0 = [str0 stringByReplacingOccurrencesOfString:@"${replace}" withString:brand];
                    if(str0 != nil)
                    {
                        [[NSUserDefaults standardUserDefaults]setObject:str0 forKey:SHOP_TITLE];
                    }
                }else{
                    if(responseObject[strtype][@"title"] != nil)
                    {
                        [user setObject:[NSString stringWithFormat:@"%@",responseObject[strtype][@"title"]] forKey:SHOP_TITLE];
                    }
                    if(responseObject[strtype][@"text"] != nil)
                    {
                        [user setObject:[NSString stringWithFormat:@"%@",responseObject[strtype][@"text"]] forKey:DISCRIPTION];
                    }
                    if(responseObject[strtype][@"icon"] != nil)
                    {
                        [user setObject:[NSString stringWithFormat:@"%@",responseObject[strtype][@"icon"]] forKey:SHOP_PIC];
                    }
                }
                
                if([strtype isEqualToString:@"spellGroup_yf"] || [strtype isEqualToString:@"h5money_yf"] || [strtype isEqualToString:@"wxdddfx"] || [strtype isEqualToString:@"indiana_yf"])
                {
                    [self gotoshare:9000 success:^{
                        [extrabonusview shareData];
                    }];
                }
...
- (void)miniShare
{
    ...
    NSString *title = @""; NSString*image = @""; NSString *path = @"/pages/shouye/shouye";
    if([sharLink isEqualToString:@"share=indiana"])
    {
...
//        image = [NSString stringWithFormat:@"%@%@!280",[NSObject baseURLStr_Upy],shop_pic];
        
        image = [NSString stringWithFormat:@"%@%@!280",[NSObject baseURLStr_Upy],_newimage];
        path = [NSString stringWithFormat:@"/pages/shouye/detail/centsIndianaDetail/centsDetail?shop_code=%@&user_id=%@",_ShopModel.shop_code,realm];
    }else if ([sharLink isEqualToString:@"share=h5money"])
    {
        title = @"50+一线大牌，2000款新款【全场1元】。错过后悔一年";
        image = @"品牌分享.jpg";
        path  = [NSString stringWithFormat:@"/pages/shouye/redHongBao?shouYePage=ThreePage&isShareFlag=true&user_id=%@",realm];
    }else
    {
        image = [NSString stringWithFormat:@"%@%@!280",[NSObject baseURLStr_Upy],shop_pic];
        title = [minishare taskrawardHttp:type2 Price:shopprice Brand:shop_brand];
...
- (void)shareSuccess
{
    [IndianaPublicModel saveShareStaue:_ShopModel.shop_code success:^(id data) {
        ...
        if(model.status == 1)
        {
            ...
            [IndianaPublicModel getShareStatuesuccess:^(id data) {
                ...
                {
                    NavgationbarView *mentionview = [[NavgationbarView alloc]init];
                    if(model.sc.intValue == 0)
                    {
                        [mentionview showLable:@"分享成功获得一次1分钱抽奖的机会" Controller:self];
                    }else{
                        
                        long long shareCount = [DataManager sharedManager].shareToNum;
                        [mentionview showLable:[NSString stringWithFormat:@"再分享%zd次即可赢得一次1分钱抽奖的机会",shareCount-model.sc.intValue] Controller:self];
                    }
                }
            }];
        }
...
//            if([sharLink isEqualToString:@"share=link"])
//            {
//                sharLink = @"";
//                [[DShareManager share] shareAppWithType:ShareTypeWeixiSession View:nil Image:shopimage WithShareType:@"Indiandetailshareshop"];
//
//            }else if([sharLink isEqualToString:@"share=indiana"]){
//                sharLink = @"";
//                [[DShareManager share] shareAppWithType:ShareTypeWeixiSession View:nil Image:shopimage WithShareType:@"Indiandshare"];
//
//            }else{
...
//            }
            ...
            if([sharLink isEqualToString:@"share=spellGroup"])
            {
                [[DShareManager share] shareAppWithType:ShareTypeWeixiSession View:nil Image:shopimage WithShareType:@"Indiandetail"];
            }else{
                [self miniShare];
                    ...
                    if(model.status == 1)
                    {
                        ...
                        [IndianaPublicModel getShareStatuesuccess:^(id data) {
                            ...
                            if(model.status == 1)
                            {
                                ...
                                if(model.sc.intValue == 0)
                                {
                                    [mentionview showLable:@"分享成功获得一次1分钱抽奖的机会" Controller:weakSelf];
                                }else{

                                    long long shareCount = [DataManager sharedManager].shareToNum;
                                    [mentionview showLable:[NSString stringWithFormat:@"再分享%zd次即可赢得一次1分钱抽奖的机会",shareCount-model.sc.intValue] Controller:weakSelf];
                                }
                            }
                        }];
                    }
