...
- (void)viewDidLoad {
    ...
    [super setNavigationItemLeft:@"夺宝晒单"];
...
- (void)createUI
{
    ...
    TFTitleView *ttv = [[TFTitleView alloc] initWithFrame:CGRectMake(0, YZERO-20, kScreenWidth, 0)];
...
//    [self.scroller addSubview:ttv];
    ...
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, [self getViewY:ttv], kScreenWidth, 1)];
    lineView.backgroundColor = COLOR_GREY(0.3);
    [self.scroller addSubview:lineView];
    
    //创建collectionView
//    [self createCollectionView:lineView];
    
    self.bgView = [[UIView alloc] initWithFrame:CGRectMake(0, [self getViewY:lineView]+5, kScreenWidth, kScreenHeight)];
...
//        if (i == 0) {
//            UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(ZOOM(60), 0, (70), ZOOM6(100))];
//            
//            label.tag = 300+i;
...
//            
//            UILabel *scalelabel = [[UILabel alloc] initWithFrame:CGRectMake(kScreenWidth-ZOOM(40)-ZOOM6(60), 0, ZOOM6(60), ZOOM6(100))];
//            scalelabel.tag = 600+i;
//            scalelabel.text = @"0分";
//            scalelabel.textColor = tarbarrossred;
//            scalelabel.font = [UIFont systemFontOfSize:ZOOM6(30)];
//            [self.bgView addSubview:scalelabel];
//            
//            UILabel *linelable = [[UILabel alloc] initWithFrame:CGRectMake(0, label.frame.origin.y+label.frame.size.height, kScreenWidth, 1)];
//            linelable.backgroundColor = kBackgroundColor;
//            [self.bgView addSubview:linelable];
//            
//        } else {
//            UILabel *ll = (UILabel *)[self.bgView viewWithTag:300];
//            UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(ZOOM(60), [self getViewY:ll]+(i-1)*ll.frame.size.height, HEIGHT(70), ll.frame.size.height)];
//            
//            label.tag = 300+i;
//            label.text = self.titleArr[i];
//            label.font = [UIFont systemFontOfSize:ZOOM6(30)];
...
//            
//            
//            UILabel *scalelabel = [[UILabel alloc] initWithFrame:CGRectMake(kScreenWidth-ZOOM(40)-ZOOM6(60), label.frame.origin.y, ZOOM6(60), ZOOM6(100))];
//            scalelabel.tag = 600+i;
//            scalelabel.text = @"0分";
//            scalelabel.textColor = tarbarrossred;
//            scalelabel.font = [UIFont systemFontOfSize:ZOOM6(30)];
...
//    }
    ...
    CGFloat H = (self.bgView.frame.size.height-[self getViewY:ll]-HEIGHT(40)-HEIGHT(10)*3-HEIGHT(35)-HEIGHT(20));
    if (INCHES_3_5) {
        H = 50;
    }
    else if(INCHES_4_0)
    ...
    self.textView = [[UITextView alloc] initWithFrame:CGRectMake(ZOOM(60), [self getViewY:ll]+(10), kScreenWidth-2*ZOOM(60), ZOOM6(100))];
    self.textView.tag = 201;
    self.textView.text = @"亲~发表一下你的获奖感言吧...";
    self.textView.layer.masksToBounds = YES;
    self.textView.layer.cornerRadius = HEIGHT(5);
...
//    [self.bgView addSubview:spacelab1];
    ...
    self.photoView = [[TFPhotoView alloc] initWithFrame:CGRectMake(self.textView.frame.origin.x, [self getViewY:spacelab1]+HEIGHT(10), self.textView.frame.size.width, ZOOM6(170))];
    self.photoView.typestr = @"夺宝晒单";
...
//        
//        RatingBar * startrb = [[RatingBar alloc] initWithFrame:CGRectMake([self getViewX:ll], ll2.frame.origin.y, ZOOM6(46)*5+ZOOM(50)*6, ZOOM6(46))];
//        startrb.tag = 8888 +j;
...
- (void)commitBtnClick:(UIButton *)sender
{
    ...
    }else if (self.imgArr.count <1)
    {
        ...
        _timeoutTimer = [NSTimer scheduledTimerWithTimeInterval:5*3 target:self selector:@selector(timeoutFunc) userInfo:nil repeats:NO];
...
- (void)httpSubmitIndianaEvalution
{
    ...
    if (self.imgArr.count == 0) { //没有图片
        ...
        urlStr = [NSString stringWithFormat:@"%@shareOrder/addComment?token=%@&version=%@&content=%@&shop_code=%@&shop_name=%@&issue_code=%@&lucky_number=%@",URLHTTP,token,VERSION,self.textView.text,shop_code,shop_name,issue_code,lucky_number];
...
- (void)httpSubmitIndianaImageEvalution
{
    ...
    urlStr = [NSString stringWithFormat:@"%@shareOrder/addComment?token=%@&version=%@&content=%@&pic=%@&shop_code=%@&shop_name=%@&issue_code=%@&lucky_number=%@",URLHTTP,token,VERSION,self.textView.text,pic,shop_code,shop_name,issue_code,lucky_number];
...
//        urlStr = [NSString stringWithFormat:@"%@shopComment/addShopComment?token=%@&version=%@&id=%@&content=%@&comment_type=%d&ordercode=%@&star=%@&color=%d&type=%d&work=%d&cost=%d",URLHTTP,token,VERSION,ID,sModel.commentText,sModel.type,order_code,sModel.starCount,sModel.colorBl,sModel.styleBl,sModel.makeBl,sModel.priceBl];
//        
//        URL = [MyMD5 authkey:urlStr];
////        //
//        AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
...
//    urlStr = [NSString stringWithFormat:@"%@shopComment/addShopComment?token=%@&version=%@&id=%@&content=%@&comment_type=%d&ordercode=%@&star=%@&color=%d&type=%d&work=%d&cost=%d&pic=%@",URLHTTP,token,VERSION,ID,sModel.commentText,sModel.type,order_code,sModel.starCount,sModel.colorBl,sModel.styleBl,sModel.makeBl,sModel.priceBl,pic];
//    
//    URL = [MyMD5 authkey:urlStr];
////    //
//    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
...
//        NSString *token = [ud objectForKey:USER_TOKEN];
//        NSString *order_code = self.Ordermodel.order_code;
//        NSMutableString *urlStr = [NSMutableString stringWithFormat:@"%@shopComment/addShopCommentList?token=%@&version=%@&ordercode=%@&jsonComment=",URLHTTP,token,VERSION,order_code];
//        NSString *URL;
...
- (void)createCollectionView:(UIView *)lineView
{
    ...
    self.collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, [self getViewY:lineView], kScreenWidth, ZOOM6(190)) collectionViewLayout:layout];
    self.collectionView.backgroundColor = [UIColor whiteColor];
    self.collectionView.pagingEnabled = YES;

    self.collectionView.frame = CGRectMake(0, [self getViewY:lineView], kScreenWidth, ZOOM6(190));
    ...
    UIView *lineView1 = [[UIView alloc] initWithFrame:CGRectMake(0, [self getViewY:self.collectionView], kScreenWidth, 1)];
    lineView1.backgroundColor = COLOR_GREY(0.3);
...
- (UIImage *)compressionImage:(UIImage *)img
{
    ...
    imagesize.height = imagesize.height/IMGSIZECOMPCOEFF;
    imagesize.width = imagesize.width/IMGSIZECOMPCOEFF;

    UIImage *newImg = [self imageWithImage:img scaledToSize:imagesize];

    NSData *imageData = UIImageJPEGRepresentation(newImg, IMGCOMPCOEFF);
...
- (NSData *)dataWithImageWithImage:(UIImage*)image scaledToSize:(CGSize)newSize;
{
    ...
    return UIImageJPEGRepresentation(newImage, IMGCOMPCOEFF);
}
