...
-(void)touchUpInside:(UIButton *)ges{
    ...
    TouchText=[NSString stringWithFormat:@"%@",ges];
    
    NSUserDefaults *usedfaul=[NSUserDefaults standardUserDefaults];
    [usedfaul setObject:TouchText forKey:@"text"];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    NSMutableString *str=[NSMutableString stringWithFormat:@"%@",touches];
    NSArray *arr=[str componentsSeparatedByString:@";"];
    if(arr.count)
    {
        text=[NSString stringWithFormat:@"%@",arr[9]];
    }
    NSArray *brr=[text componentsSeparatedByString:@"="];
    if(brr.count)
    {
        touchtext=brr[1];
...
//        [delegate performSelector:@selector(touchUpInside:) withObject:touchtext];
//    }else{
//        [delegate performSelector:@selector(touchOut:) withObject:self];
//    }
    
    NSUserDefaults *usedfaul=[NSUserDefaults standardUserDefaults];
    NSString *selecttext=[usedfaul objectForKey:@"text"];

    if ([touchtext isEqualToString:selecttext]) {
        if(touchtext)
        {
            [delegate performSelector:@selector(touchUpInside:) withObject:touchtext];
        }
