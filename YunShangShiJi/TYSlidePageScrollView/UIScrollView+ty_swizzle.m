...
+ (BOOL)ty_swizzleMethodWithOrignalSel:(SEL)originalSel replacementSel:(SEL)replacementSel
{
    ...
    if (!origMethod) {
        //original method %@ not found for class %@", NSStringFromSelector(originalSel), [self class]);
        return NO;
    }
    
    if (!replMethod) {
        //replace method %@ not found for class %@", NSStringFromSelector(replacementSel), [self class]);
        return NO;
    }
