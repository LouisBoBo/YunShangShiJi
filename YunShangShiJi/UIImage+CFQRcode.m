...
+ (void)qrIamge:(UIImage *)qrImage addBgImage:(UIImage *)bgImage bgImageSize:(CGFloat) bgImageSize completion:(void (^)(UIImage *image))completion{
    [UIImage qrIamge:qrImage qrImageType:CenterImgType_SquareAndBottom addBgImage:bgImage bgImageSize:bgImageSize completion:completion];
}
...
+ (void)qrIamge:(UIImage *)qrImage qrImageType:(CenterImgType)type addBgImage:(UIImage *)bgImage bgImageSize:(CGFloat) bgImageSize completion:(void (^)(UIImage *image))completion{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        bgImg = [self imageCompressForSize:bgImage targetSize:bgImage.size];
...
+ (UIImage *)qrcodeImage:(UIImage *)qrImage addIconImage:(UIImage *)iconImage centerImageType:(CenterImgType) type scale:(CGFloat)scale {
   ...
   UIGraphicsBeginImageContextWithOptions(rect.size, YES, screenScale);
    ...
    if(iconImage){ // 如果有图片
        ...
        CGSize avatarSize = CGSizeMake(rect.size.width * scale, rect.size.width * scale);
        ...
        }else if (type == CenterImgType_SquareAndBottom){
            y = (rect.size.height - avatarSize.height);
        }else if (type == CenterImgType_SquareAndRightBottom){
            x = (rect.size.width - avatarSize.width-10);
            y = (rect.size.height - avatarSize.height-30);
        }
