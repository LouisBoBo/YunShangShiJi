...
+ (BOOL)isTableCreated
{
    return [[self getUsingLKDBHelper] getTableCreatedWithClass:self];
}
...
+ (NSInteger)sumWidthVariable:(NSString *)variable where:(id)where, ...
{
    if ([where isKindOfClass:[NSString class]]) {
        va_list list;
        va_start(list, where);
    ...
    return [[self getUsingLKDBHelper] sum:self variable:variable where:where];
}
