...
@interface ShopDetailViewController ()<DShareManagerDelegate,UISearchBarDelegate, TFScreeningBackgroundDelegate,MiniShareManagerDelegate>
...
@property (nonatomic, assign) BOOL is_fightTask;   //是否是拼团任务
@property (nonatomic, assign) BOOL is_freelingTask;//是否是免费领任务
...
@property (nonatomic, strong) vipInfoModel *vipInfoModel;

@property (nonatomic, assign) CGFloat shop_deduction;  //折扣率
@property (nonatomic, strong) NSString *maxType;       //最高会员等级
@property (nonatomic, assign) NSInteger is_vipPage;    //是否是会员界面
@property (nonatomic, assign) NSInteger is_vip;        //是否是会员
@property (nonatomic, assign) NSInteger vip_free;      //会员免费领
@property (nonatomic, assign) NSInteger ordervip_free; //会员免费领
@property (nonatomic, assign) NSInteger viptype;
@property (nonatomic, assign) NSInteger firstGroup;
@property (nonatomic, assign) NSInteger is_buy;        //是否要购买会员
@property (nonatomic, assign) BOOL isfreeLingClick;    //是否点击会员免费领
@property (nonatomic, strong) UILabel *moneylabel;
@property (nonatomic, assign) NSInteger is_open;       //为他人分享是否打开1开 2不开
@property (nonatomic, strong) NSString *share_userid;
@property (nonatomic, assign) double ElastTime;
@property (nonatomic, strong ) NSTimer *hongbaotimer;
...
//    NSMutableArray *_ImageDataArray;
    ...
    NSString *_shop_fight_price; //g开团价格
...
-(void)creatHeadview
{
    ...
    _siimage = [[UIImageView alloc]initWithFrame:CGRectMake((_sharebtn.frame.size.width-IMGSIZEW(@"icon_xihuan")*imagescale)/2, (_sharebtn.frame.size.height-IMGSIZEH(@"icon_xihuan")*imagescale)/2, IMGSIZEW(@"icon_xihuan")*imagescale, IMGSIZEH(@"icon_xihuan")*imagescale)];
...
//        NSURL *imgUrl = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@%@",[NSObject baseURLStr_Upy],_newimage,st]];
        ...
        {
            UIImageView *shareMoneyImage = [[UIImageView alloc]initWithFrame:CGRectMake(kScreenWidth-ZOOM6(230), ZOOM6(300), ZOOM6(230), ZOOM6(60))];
            ...
            CGFloat wenhaospace = self.is_open == 1 ?0 : ZOOM6(60);
            UILabel *moneylabel = [[UILabel alloc]initWithFrame:CGRectMake(0, -ZOOM6(3), CGRectGetWidth(shareMoneyImage.frame)-wenhaospace, CGRectGetHeight(shareMoneyImage.frame))];
//            moneylabel.text = [NSString stringWithFormat:@"分享赚%.1f元",[_ShopModel.shop_se_price floatValue]*0.1];
            ...
            [shareMoneyImage addSubview:self.moneylabel = moneylabel];
            ...
            UIImageView *wenhaoimg = [[UIImageView alloc]initWithFrame:CGRectMake(CGRectGetWidth(shareMoneyImage.frame)-ZOOM6(60), (CGRectGetHeight(shareMoneyImage.frame)-ZOOM6(40))/2-ZOOM6(2), ZOOM6(40), ZOOM6(40))];
            wenhaoimg.userInteractionEnabled = YES;
            wenhaoimg.clipsToBounds = YES;
            wenhaoimg.layer.cornerRadius = CGRectGetHeight(wenhaoimg.frame)/2;
            wenhaoimg.layer.borderWidth = 1;
            wenhaoimg.layer.borderColor = [UIColor whiteColor].CGColor;
            wenhaoimg.image = [UIImage imageNamed:@"share_wenhao.png"];
            self.is_open == 1?nil:[shareMoneyImage addSubview:wenhaoimg];
            
            UITapGestureRecognizer *wentap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(wenhaoShare)];
            [wenhaoimg addGestureRecognizer:wentap];
        }
        
//        CGFloat headimageYY=CGRectGetMaxY(self.Headimage.frame);
        CGFloat headimageYY=700*kApplicationWidth/600;
        self.backview.frame=CGRectMake(0, headimageYY, kApplicationWidth, self.backview.frame.size.height);
...
//    self.se_price.text=[NSString stringWithFormat:@"￥%.1f",[self.stringtype isEqualToString:@"活动商品"]?[_ShopModel.shop_se_price floatValue]:[_ShopModel.shop_se_price floatValue]*0.9];
    ...
    CGFloat shop_se_price = [_ShopModel.shop_se_price floatValue]-[DataManager sharedManager].OneYuan_count;
    ...
    if([self.stringtype isEqualToString:@"活动商品"] && !self.isNOFightgroups)
    {
        ...
        if(self.is_vip >0)
        {
            ...
            self.se_price.text = [NSString stringWithFormat:@"%.1f",self.app_shop_group_price.floatValue];
        }
    }
...
//        self.priceline.frame=CGRectMake(CGRectGetMaxX(self.se_price.frame)+2,self.oldprice.frame.origin.y + self.oldprice.frame.size.height/2, textSize.width-4, self.priceline.frame.size.height);
        ...
        self.DeductibleBtn.frame = CGRectMake(CGRectGetMaxX(self.discountlable.frame)+ZOOM6(10), CGRectGetMinY(self.se_price.frame)-ZOOM6(4), ZOOM6(240), CGRectGetHeight(self.se_price.frame)+ZOOM6(8));
...
//        [self.backview addSubview:self.ManufacturerImage];
    ...
    CGFloat discriptionlabHeigh = (kScreenWidth)/5.3;
    [self.fwcnview removeFromSuperview];
    self.fwcnview = [[UIImageView alloc]init];
    self.fwcnview.frame = CGRectMake(0, lineY, kScreenWidth, discriptionlabHeigh);
//    self.fwcnview.image = [UIImage imageNamed:@"服务承诺fwcn"];
    [self.fwcnview sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",[NSObject baseURLStr_XCX_Upy],@"small-iconImages/heboImg/fuwuchengnuo_newest.png"]]];
    [self.backview addSubview:self.fwcnview];
...
//    lineY +=discriptionlabHeigh;
    ...
    if(self.is_open == 1)
    {
        self.moneylabel.text = @"为他人分享";
    }else{
        self.moneylabel.text = [NSString stringWithFormat:@"卖赚%.1f元",_ShopModel.shop_se_price.floatValue*0.1];
    }
...
-(void)creatSectionView
{
    ...
    NSArray *titleArr= self.isTM ?@[@"商品详情"] : @[@"商品详情",@"参数",comstr];
...
-(void)creatFootView
{
    ...
    }else if ([DataManager sharedManager].is_OneYuan)
    {
        ...
        {
            arr=@[@"0元购买",@""];
        }else if (self.is_fightTask){
            arr = @[@"一键开团"];
        }else if (self.is_freelingTask){
            arr = @[@"免费领任务"];
        }
        else{
            arr=@[@"拼团疯抢",@""];
    ...
    for(int i=0;i<arr.count;i++)
    {
        ...
        if(_isMonday == YES || ![DataManager sharedManager].is_OneYuan || self.is_fightTask || self.is_freelingTask)
        {
        ...
        contactbtn.titleLabel.font = [UIFont systemFontOfSize:ZOOM6(28)];
        ...
        if(i==0)//右边按钮
        {
            if(token == nil)
            {
                [contactbtn setBackgroundImage:[UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",[NSObject baseURLStr_XCX_Upy],@"small-iconImages/heboImg/becomeMember_freeling.jpg"]]]] forState:UIControlStateNormal];
            }else{
                if(_isMonday == YES)
                {
                ...
                }else if (![DataManager sharedManager].is_OneYuan)
                {
                    ...
                    pubstr = [NSString stringWithFormat:@"￥%.1f",[_shop_fight_price floatValue]];
                    [contactbtn setTitle:[NSString stringWithFormat:@"￥%.1f\n 一键开团",[_shop_fight_price floatValue]] forState:UIControlStateNormal];
                }else if (self.is_freelingTask){
                    
                    [contactbtn setImage:[UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",[NSObject baseURLStr_XCX_Upy],@"small-iconImages/heboImg/becomeMember_freeling.jpg"]]]] forState:UIControlStateNormal];
                    contactbtn.imageView.contentMode = UIViewContentModeScaleAspectFit;
                }
                else if (self.is_vip >= 1)//单独购买
                {
                    pubstr = [NSString stringWithFormat:@"￥%.1f",[_ShopModel.shop_se_price floatValue]];
                ...
                else if (self.is_vip == 0)//会员免费领
                {
                    [contactbtn setBackgroundImage:[UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",[NSObject baseURLStr_XCX_Upy],@"small-iconImages/heboImg/becomeMember_freeling.jpg"]]]] forState:UIControlStateNormal];
                }
                else{
                    
                    pubstr = [NSString stringWithFormat:@" ￥%.2f",[_ShopModel.app_shop_group_price floatValue]];
                    if(self.isNewbie)
                    {
            ...
            if(token == nil)
            {
                ...
                if(self.is_vip >=1)//会员免费领
                {
                    [contactbtn setBackgroundImage:[UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",[NSObject baseURLStr_XCX_Upy],@"small-iconImages/heboImg/newestbecomeMember_freeling.jpg"]]]] forState:UIControlStateNormal];
                }
                else if(self.is_vip ==0)//不是会员去拼团
                {
                    pubstr = [NSString stringWithFormat:@" ￥%.2f",[_ShopModel.app_shop_group_price floatValue]];
        ...
        if([self.stringtype isEqualToString:@"活动商品"])
        {
            ...
            if(i==0)
            {
                if(self.isNOFightgroups == YES)
                {
                    ...
                    pubstr = [NSString stringWithFormat:@"   ￥%.2f",[_ShopModel.app_shop_group_price floatValue]];
                    [contactbtn setTitle:[NSString stringWithFormat:@"   ￥%.2f\n%@人成团",[_ShopModel.app_shop_group_price floatValue],[Signmanager SignManarer].rnum] forState:UIControlStateNormal];
                }
...
- (void)creatShareModelView:(NSString*)imageurl
{
    ...
    {
        titlelable1.text = @"邀请4位好友，即可免费领走本商品";
        ...
        [noteStr addAttribute:NSForegroundColorAttributeName value:tarbarrossred range:NSMakeRange(2, str.length)];
        [noteStr addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:ZOOM6(56)] range:NSMakeRange(2, str.length)];
        ...
        if (responseObject!=nil) {
            ...
            if(statu.intValue==1)//请求成功
            {
                ...
                if(![_shopDic isEqual:[NSNull null]])
                {
                    ...
                    model.assmble_price =[NSString stringWithFormat:@"%@",responseObject[@"shop"][@"assmble_price"]];
                    ...
                    _shop_fight_price = [NSString stringWithFormat:@"%@",responseObject[@"shop"][@"assmble_price"]];
                    ...
                    model.app_shop_group_price = self.app_shop_group_price;
                    _ShopModel=model;
                }
...
- (void)getOneYuanCount
{
    ...
    [OneYuanModel GetOneYuanCountSuccess:^(id data) {
        ...
        if(oneModel.status == 1)
        {
            ...
            weakself.is_open = oneModel.is_open;
...
//                }
        ...
        [weakself getvip_userInfoData];
    }];
}
//获取用户会员相关信息
- (void)getvip_userInfoData
{
    NSInteger t = 1;
    NSInteger page3 = self.is_freelingTask?1:0;
    kWeakSelf(self);
    [vipInfoModel Getvip_userInfoDataSuccess:self.shop_code T:t Page:page3 Success:^(id data) {
        self.vipInfoModel = [vipInfoModel new];
        self.vipInfoModel = data;
        if(self.vipInfoModel.status == 1)
        {
            
            self.is_vipPage = self.vipInfoModel.vip_page;
            self.is_vip = self.vipInfoModel.isVip;
            self.is_buy = self.vipInfoModel.is_buy;
            self.viptype = self.vipInfoModel.vip_type;
            self.firstGroup = self.vipInfoModel.first_group;
            self.vip_free = self.vipInfoModel.vip_free;
            self.ordervip_free = self.vipInfoModel.vip_free;
            
            [weakself refreshBigImageview];
        
        }else{
            
            if([self.stringtype isEqualToString:@"新人钜惠"])
            {
                self.vip_free = 2;
            }
        }
...
{
    kWeakSelf(self);
    VitalityTaskPopview* vitaliview = [[VitalityTaskPopview alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight) VitalityType:Share_Deductible valityGrade:0 YidouCount:0];
...
- (void)getShareImage:(BOOL)isPopView
{
...
-(void)gotoshare:(int)sharetag
{
    ...
    if(sharetag==9000)//微信好友
    {
        ...
        NSString *share_userid = (self.share_userid !=nil && [self.share_userid isEqual:[NSNull null]])?self.share_userid:realm;
        NSString *path  = [NSString stringWithFormat:@"/pages/shouye/detail/detail?shop_code=%@&user_id=%@",_ShopModel.shop_code,share_userid];

        NSString *type2 = [user objectForKey:SHOP_TYPE2];
        type2 = ![type2 isEqualToString:@"(null)"]?type2:@"ZARA";
        
        NSString *title = [NSString stringWithFormat:@"快来%.1f元拼【%@正品%@】专柜价%.2f元！",[DataManager sharedManager].app_value,_ShopModel.supp_label,type2,[_ShopModel.shop_se_price floatValue]];
...
//        [minishare shareAppWithType:MINIShareTypeWeixiSession Image:image Title:title Discription:nil WithSharePath:path];
        ...
        UIImage *afterImage = [pi getShareImage:zhezhaoImg WithBaseImg:baseImg WithPrice:(NSString*)_ShopModel.app_shop_group_price];
        NSData *imageData = UIImageJPEGRepresentation(afterImage,0.8f);
        
        title= [NSString stringWithFormat:@"点击购买👆【%@】今日特价%.1f元！",_ShopModel.shop_name,[_ShopModel.shop_se_price floatValue]];
...
- (void)colorAndsizeModelview:(BOOL)miniShareSuccess
{
    if(self.isTM)
    {
        [self specialcolorAndsizeModelview:miniShareSuccess];
        return;
    }
    
    NSString *shop_group_price = @"";
    if([self.stringtype isEqualToString:@"活动商品"] && self.tag == 3001 && !self.isNOFightgroups)//2人成团下单
    {
        shop_group_price = _shop_group_price;
    }
    
    if(_is_oneLuck_share){
        shop_group_price = self.app_shop_group_price;
    }else if (_is_fightTask){
        shop_group_price = _shop_fight_price;
    }else{
        CGFloat mymoney = [self.viewmodel get_discount_price:_ShopModel.shop_se_price.floatValue DiscountMoney:[DataManager sharedManager].one_not_use_price MaxViptype:self.maxType Shop_deduction:self.shop_deduction];
        shop_group_price = [NSString stringWithFormat:@"%.1f",mymoney];
    }
    _ShopModel.app_shop_group_price = shop_group_price;
    
    ChangeShopPopview* shopPopview = [[ChangeShopPopview alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight) ShopModel:_ShopModel];
    shopPopview.isNewbie = self.isNewbie;
    shopPopview.isfreeLingClick = self.isfreeLingClick;
...
//            if([self.stringtype isEqualToString:@"活动商品"])
//            {
//                ActivityShopOrderVC *affirm=[[ActivityShopOrderVC alloc]init];
//                _ShopModel.stock_type_id=changepmodel.stock_type_id;
//                _ShopModel.original_price=changepmodel.original_price;
//                _ShopModel.shop_num =changepmodel.shop_num;
//                _ShopModel.shop_color = changepmodel.shop_color;
...
//                if(self.tag == 3001)
//                {
//                    if(self.isNOFightgroups)//是活动商品但不是拼团
//                    {
//                        affirm.isNOFightgroups =self.isNOFightgroups;
//                        affirm.shopModel.shop_se_price=_shop_se_price;
//                    }else{//2人成团下单
//                        BOOL isfound = false;
//                        NSMutableArray *newdataArray = [NSMutableArray array];
//                        for(int i =0;i<[DataManager sharedManager].fightData.count;i++)
//                        {
//                            ShopDetailModel *oldmodel = [DataManager sharedManager].fightData[i];
//                            if([_ShopModel.shop_code isEqualToString:oldmodel.shop_code])
//                            {
//                                oldmodel = _ShopModel;
//                                isfound = YES;
//                            }
//                            [newdataArray addObject:oldmodel];
//                        }
//                        
//                        if(!isfound)
//                        {
//                            [[DataManager sharedManager].fightData addObject:_ShopModel];
//                        }else{
//                            [[DataManager sharedManager].fightData removeAllObjects];
//                            [[DataManager sharedManager].fightData addObjectsFromArray:newdataArray];
//                        }
//                        
//                        if([DataManager sharedManager].fightData.count == 2)
//                        {
//                            [self.fightDataArray addObjectsFromArray:[DataManager sharedManager].fightData];
//                            [self performSelector:@selector(fightpay) withObject:self afterDelay:0.5];
//                        }else{
//                            if(self.FightselectBlock)
//                            {
//                                self.FightselectBlock();
//                            }
...
//            }
            ...
            if(self.is_fightTask || self.is_freelingTask || _is_oneLuck_share)
            {                
                 [self performSelector:@selector(fightpay) withObject:self afterDelay:0.5];
            }else{
                NSMutableArray *shopArray = [NSMutableArray array];
                ...
                view.isFight = NO;
                view.isTM = NO;
                view.vipType = self.viptype;
                view.vip_free = self.ordervip_free > 0 ? 1 : 0;
                view.max_vipType = self.maxType.integerValue;
                view.vip_page = self.is_vipPage;
                view.firstGroup = self.firstGroup;
                view.affirmOrder=^{
                    [self changeTabbarCartNum];
                };
                [self.navigationController pushViewController:view animated:YES];
...
#pragma mark 颜色尺码选择框
- (void)specialcolorAndsizeModelview:(BOOL)miniShareSuccess
{
    NSString *shop_group_price = @"";
...
- (void)popViewWithDrawCashExplain {
    ...
    popView.rightText = @"提交并分享";
    popView.title = @"填写邀请码";
    popView.titleLabel.textAlignment = NSTextAlignmentLeft;
    ...
    contentLabel.text = @"邀请码";
    contentLabel.userInteractionEnabled = YES;
    [contentV addSubview:contentLabel];
    
    UIImageView *codedisbtn = [[UIImageView alloc]init];
    codedisbtn.image = [UIImage imageNamed:@"share_wenhao.png"];
    codedisbtn.userInteractionEnabled = YES;
    [contentV addSubview:codedisbtn];
    
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(codeShare)];
    [contentLabel addGestureRecognizer:tap];
    [codedisbtn addGestureRecognizer:tap];
    
    UITextField *nameTextField = [[UITextField alloc] init];
    nameTextField.borderStyle = UITextBorderStyleRoundedRect;
    nameTextField.placeholder = @"请输入邀请码";
    ...
    [codedisbtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(contentLabel);
        make.left.equalTo(contentLabel).offset(ZOOM6(100));
        make.height.mas_offset(ZOOM6(35));
        make.width.mas_offset(ZOOM6(35));
    }];
    ...
    [idenfTextField mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(nameTextField.mas_bottom).offset(ZOOM6(0));
        make.left.right.equalTo(contentV);
    ...
    [popView showCancelBlock:^{
        ...
        weakself.share_userid = nameText;
...
- (void)codeShare{
    ...
    VitalityTaskPopview* vitaliview = [[VitalityTaskPopview alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight) VitalityType:CodeInvite_what valityGrade:0 YidouCount:0];
    __weak VitalityTaskPopview *view = vitaliview;
        ...
        if(self.slectbtn.tag==1000)
        {
            ...
            }else if (indexPath.row <self.dataArr.count+1)
            {
                if(indexPath.row <self.dataArr.count)
                {
                    ...
                    NSURL *imgUrl = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@!450",[NSObject baseURLStr_Upy],self.dataArr[indexPath.row-1]]];
                    UIImage *image = [UIImage imageWithData:[NSData dataWithContentsOfURL:imgUrl]];
                    return image.size.height*0.87;
                }
            }
            else{
            ...
            if(strongSelf.SizeDataArray.count || self.sizeArray.count)
            {
                if([self.stringtype isEqualToString:@"活动商品"])
                {
                    ...
                    }else if (![DataManager sharedManager].is_OneYuan)//单独购买
                    {
                        [strongSelf colorAndsizeModelview:NO];
                        return;
                    }else if (self.is_fightTask){//开团
                        [strongSelf colorAndsizeModelview:NO];
                        return;
                    }else if (self.is_freelingTask){//免费领任务
                        
                        self.isfreeLingClick = YES;
                        ...
                        if(sender.tag == 3002)//左边按钮
                        {
//                            if(self.vip_free == 2)
//                            {
//                                _is_oneLuck_share = YES;
//                            }else if (self.vip_free == 1)
//                            {
//                                _is_oneLuck_share = YES;
//                            }else if (self.is_vipPage == 1 && self.is_vip>=1)
//                            {
//                                self.isfreeLingClick = YES;
//                                _is_oneLuck_share = YES;
//                            }else if (self.is_vipPage == 1 && self.is_vip==0)
//                            {
//                                _is_oneLuck_share = YES;
//                            }else{
//                                _is_oneLuck_share = YES;
...
//                            }
                            ...
                            self.isfreeLingClick = self.vip_free > 0?YES:NO;
                            
                            _is_oneLuck_share = self.is_vip > 0 ?NO:YES;
                ...
                if(_is_oneLuck_share)//拼团
                {
                    //is_buy==1去购买会员卡 vip_type<0去补卡
//                    if(self.is_buy == 1 && self.vip_free !=1)
//                    {
...
//
//                        return;
//                    }else if(self.viptype <= 0 && self.vip_free !=1){
//                        AddMemberCardViewController *member = [[AddMemberCardViewController alloc]init];
//                        member.hidesBottomBarWhenPushed = YES;
//                        member.from_vipType = @"detail";
//                        member.vip_type = [NSString stringWithFormat:@"%zd",self.viptype];
//                        [self.navigationController pushViewController:member animated:YES];
//
//                        return;
//                    }
                    
                    //是会员完成当天任务去免费领，非会员分享4次后去免费领
                    if(self.is_vip > 0 && self.is_vip !=3)
                    {
                        
                    }else{
                        [self getShareImage:YES];
                        return;
                    }
                    
                    if(self.vip_free == 1 || self.viptype >0)
                    {
                        [strongSelf colorAndsizeModelview:NO];
                    }else{
                        AddMemberCardViewController *member = [[AddMemberCardViewController alloc]init];
...
-(void)fightpay
{
    ...
    if(shopArray.count){
        ...
        [DataManager sharedManager].app_value = self.app_shop_group_price.floatValue;
        
        OrderTableViewController *view = [[OrderTableViewController alloc]init];
        view.hidesBottomBarWhenPushed=YES;
        view.orderBuyType = self.is_fightTask?OrderType_FightBuy:OrderType_GroupBuy;
        view.sortArray = shopArray;
...
{
    if(_vipInfoModel == nil)
    {
        _vipInfoModel = [vipInfoModel alloc];
    }
...
- (void)creatRedHongBaoview
{
    ...
    self.redhongbaoview.closeHongBaoBlack = ^(double lastTime) {
        ...
        //倒计时
        weakself.hongbaotimer= [NSTimer weakTimerWithTimeInterval:weakself.ElastTime target:weakself selector:@selector(creatRedHongBaoview) userInfo:nil repeats:YES];
    };
